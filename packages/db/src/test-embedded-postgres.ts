...
async function probeEmbeddedPostgresSupport(): Promise<EmbeddedPostgresTestSupport> {
  const dataDir = fs.mkdtempSync(path.join(os.tmpdir(), "handrails-embedded-postgres-probe-"));
...
export async function startEmbeddedPostgresTestDatabase(
  ...
  try {
    ...
    const connectionString = `postgres://handrails:handrails@127.0.0.1:${port}/handrails`;
