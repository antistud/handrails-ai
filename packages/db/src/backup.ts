...
function resolveHandrailsInstanceId(): string {
  const raw = process.env.HANDRAILS_INSTANCE_ID?.trim() || "default";
...
function resolveDefaultConfigPath(): string {
  return path.resolve(resolveHandrailsHomeDir(), "instances", resolveHandrailsInstanceId(), "config.json");
}
...
function resolveDefaultBackupDir(): string {
  return path.resolve(resolveHandrailsHomeDir(), "instances", resolveHandrailsInstanceId(), "data", "backups");
}
