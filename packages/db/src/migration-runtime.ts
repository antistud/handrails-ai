...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://handrails:handrails@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "handrails");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://handrails:handrails@127.0.0.1:${preferredPort}/handrails`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://handrails:handrails@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "handrails");
    return {
      connectionString: `postgres://handrails:handrails@127.0.0.1:${port}/handrails`,
  ...
  const adminConnectionString = `postgres://handrails:handrails@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "handrails");

  return {
    connectionString: `postgres://handrails:handrails@127.0.0.1:${selectedPort}/handrails`,
