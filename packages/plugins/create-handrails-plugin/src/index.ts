...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".handrails-sdk");
...
/**
 * Generate a complete Handrails plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A Handrails plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "handrails-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["handrails", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@handrailsai/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@handrailsai/plugin-sdk": sdkDependency,
...
`,
  ...
  writeFile(
    ...
    `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@handrailsai/plugin-sdk/ui";
...
${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@handrailsai/plugin-sdk\` and \`@handrailsai/shared\` from a local Handrails checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.handrails-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}
...
function runCli() {
  ...
  if (!pluginName) {
    ...
    console.error("Usage: create-handrails-plugin <name> [--template default|connector|workspace] [--output <dir>] [--sdk-path <handrails-sdk-path>]");
    process.exit(1);
  }
