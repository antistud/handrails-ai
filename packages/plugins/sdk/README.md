# `@handrailsai/plugin-sdk`

Official TypeScript SDK for Handrails plugin authors.

- **Worker SDK:** `@handrailsai/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@handrailsai/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@handrailsai/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@handrailsai/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@handrailsai/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@handrailsai/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@handrailsai/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@handrailsai/plugin-sdk/ui/hooks` | Hooks only |
| `@handrailsai/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@handrailsai/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@handrailsai/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@handrailsai/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@handrailsai/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@handrailsai/plugin-sdk/types` | Worker context and API types (advanced) |
...
```bash
pnpm add @handrailsai/plugin-sdk
```
...
- Plugin UI bundles run as same-origin JavaScript inside the main Handrails app. They can call ordinary Handrails HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the Handrails instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@handrailsai/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@handrailsai/plugin-sdk`.
...
A card or section rendered on the main dashboard. Use this for at-a-glance metrics, status indicators, or summary views that surface plugin data alongside core Handrails information. Receives `PluginWidgetProps` with `context.companyId` set to the active company. Requires the `ui.dashboardWidget.register` capability.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@handrailsai/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@handrailsai/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@handrailsai/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@handrailsai/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@handrailsai/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@handrailsai/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@handrailsai/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@handrailsai/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@handrailsai/plugin-sdk/ui";
...
import {
  useHostContext,
  usePluginAction,
} from "@handrailsai/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@handrailsai/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@handrailsai/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@handrailsai/plugin-sdk/bundlers";
...
```bash
handrails-plugin-dev-server --root . --ui-dir dist/ui --port 4177
```

Or programmatically:

```ts
import { startPluginDevServer } from "@handrailsai/plugin-sdk/dev-server";
const server = await startPluginDevServer({ rootDir: process.cwd() });
```

Dev server endpoints:
- `GET /__handrails__/health` returns `{ ok, rootDir, uiDir }`
- `GET /__handrails__/events` streams `reload` SSE events on UI build changes
