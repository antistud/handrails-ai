/**
 * Core types for the Handrails plugin worker-side SDK.
 *
 * These types define the stable public API surface that plugin workers import
 * from `@handrailsai/plugin-sdk`.  The host provides a concrete implementation
...
// ---------------------------------------------------------------------------
// Re-exports from @handrailsai/shared (plugin authors import from one place)
// ---------------------------------------------------------------------------
...
export interface ScopeKey {
  /** What kind of Handrails object this state is scoped to. */
...
export interface PluginEntityUpsert {
  ...
  externalId?: string;
  /** Human-readable title for display in the Handrails UI. */
  title?: string;
...
/**
 * `ctx.events` — subscribe to and emit Handrails domain events.
 *
...
export interface PluginEventsClient {
  /**
   * Subscribe to a core Handrails domain event or a plugin-namespaced event.
   *
 ...
 * This client resolves the reference through the Handrails secret provider
...
export interface PluginSecretsClient {
   ...
   * in the Handrails secret provider (e.g. `"MY_API_KEY"`).
   *
 ...
 * ```
 *
 * @see usePluginStream in `@handrailsai/plugin-sdk/ui`
 */
 ...
 * @example
 * ```ts
 * import { definePlugin } from "@handrailsai/plugin-sdk";
 *
