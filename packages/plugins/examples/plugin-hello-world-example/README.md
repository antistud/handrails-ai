# @handrailsai/plugin-hello-world-example
...
- a minimal React widget rendered in the Handrails dashboard
...
```bash
pnpm --filter @handrailsai/plugin-hello-world-example build
pnpm handrailsai plugin install ./packages/plugins/examples/plugin-hello-world-example
```
...
- **Reinstall after pulling.** If you installed a plugin by local path before the server stored `package_path`, the plugin may show status **error** (worker not found). Uninstall and install again so the server persists the path and can activate the plugin:  
  `pnpm handrailsai plugin uninstall handrails.hello-world-example --force` then  
  `pnpm handrailsai plugin install ./packages/plugins/examples/plugin-hello-world-example`.
