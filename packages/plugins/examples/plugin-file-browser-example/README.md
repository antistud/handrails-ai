...
Example Handrails plugin that demonstrates:
...
```bash
pnpm --filter @handrailsai/plugin-file-browser-example build
pnpm handrailsai plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm handrailsai plugin uninstall handrails-file-browser-example --force
```
...
- Optional: use `handrails-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
