...
type HandrailsSprite = (typeof HANDRAILS_SPRITES)[number];
...
function spriteSize(sprite: HandrailsSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = HANDRAILS_SPRITES[(baseCol + baseRow) % HANDRAILS_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = HANDRAILS_SPRITES[Math.floor(Math.random() * HANDRAILS_SPRITES.length)]!;
