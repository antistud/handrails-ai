...
Use when:
- You want Handrails to run Pi (the AI coding agent) locally as the agent runtime
...
- Handrails requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/handrailss/ and resumed with --session.
