...
import type { AdapterModel } from "@handrailsai/adapter-utils";
import { asString, runChildProcess } from "@handrailsai/adapter-utils/server-utils";
...
function resolvePiCommand(input: unknown): string {
  const envOverride =
    typeof process.env.HANDRAILS_PI_COMMAND === "string" &&
    process.env.HANDRAILS_PI_COMMAND.trim().length > 0
      ? process.env.HANDRAILS_PI_COMMAND.trim()
      : "pi";
  return asString(input, envOverride);
}

const discoveryCache = new Map<string, { expiresAt: number; models: AdapterModel[] }>();
const VOLATILE_ENV_KEY_PREFIXES = ["HANDRAILS_", "npm_", "NPM_"] as const;
