...
Use when:
- You want Handrails to run Cursor Agent CLI locally as the agent runtime
...
- Handrails auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$handrails" and related skills on local runs.
- Handrails auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
`;
