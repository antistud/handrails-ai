...
This document describes how `@handrailsai/adapter-openclaw-gateway` invokes OpenClaw over the Gateway protocol.
...
5. stream `event agent` frames into Handrails logs/transcript parsing
...
- required fields:
  ...
  - `idempotencyKey` (Handrails `runId`)
