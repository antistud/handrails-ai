...
## Scope
This plan is now **gateway-only**. Handrails supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Handrails skill.
...
3. In `/new` OpenClaw session, OpenClaw can still create a Handrails task.
...
```bash
pnpm --filter @handrailsai/server test:run -- openclaw-gateway
pnpm --filter @handrailsai/server typecheck
pnpm --filter @handrailsai/ui typecheck
pnpm --filter handrailsai typecheck
```
