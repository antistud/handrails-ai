...
} from "@handrailsai/adapter-utils";
import { asNumber, asString, buildHandrailsEnv, parseObject } from "@handrailsai/adapter-utils/server-utils";
...
const DEFAULT_CLIENT_VERSION = "handrails";
...
function resolveSessionKey(input: {
  ...
  const fallback = input.configuredSessionKey ?? "handrails";
  if (input.strategy === "run") return `handrails:run:${input.runId}`;
  if (input.strategy === "issue" && input.issueId) return `handrails:issue:${input.issueId}`;
  return fallback;
}
...
function resolveHandrailsApiUrlOverride(value: unknown): string | null {
...
function buildHandrailsEnvForWake(ctx: AdapterExecutionContext, wakePayload: WakePayload): Record<string, string> {
  const handrailsApiUrlOverride = resolveHandrailsApiUrlOverride(ctx.config.handrailsApiUrl);
  const handrailsEnv: Record<string, string> = {
    ...buildHandrailsEnv(ctx.agent),
    HANDRAILS_RUN_ID: ctx.runId,
  };

  if (handrailsApiUrlOverride) {
    handrailsEnv.HANDRAILS_API_URL = handrailsApiUrlOverride;
  }
  if (wakePayload.taskId) handrailsEnv.HANDRAILS_TASK_ID = wakePayload.taskId;
  if (wakePayload.wakeReason) handrailsEnv.HANDRAILS_WAKE_REASON = wakePayload.wakeReason;
  if (wakePayload.wakeCommentId) handrailsEnv.HANDRAILS_WAKE_COMMENT_ID = wakePayload.wakeCommentId;
  if (wakePayload.approvalId) handrailsEnv.HANDRAILS_APPROVAL_ID = wakePayload.approvalId;
  if (wakePayload.approvalStatus) handrailsEnv.HANDRAILS_APPROVAL_STATUS = wakePayload.approvalStatus;
  if (wakePayload.issueIds.length > 0) {
    handrailsEnv.HANDRAILS_LINKED_ISSUE_IDS = wakePayload.issueIds.join(",");
  }

  return handrailsEnv;
}

function buildWakeText(payload: WakePayload, handrailsEnv: Record<string, string>): string {
  const claimedApiKeyPath = "~/.openclaw/workspace/handrails-claimed-api-key.json";
  const orderedKeys = [
    "HANDRAILS_RUN_ID",
    "HANDRAILS_AGENT_ID",
    ...
    "HANDRAILS_API_URL",
    "HANDRAILS_TASK_ID",
    "HANDRAILS_WAKE_REASON",
    "HANDRAILS_WAKE_COMMENT_ID",
    "HANDRAILS_APPROVAL_ID",
    "HANDRAILS_APPROVAL_STATUS",
    "HANDRAILS_LINKED_ISSUE_IDS",
  ];

  const envLines: string[] = [];
  for (const key of orderedKeys) {
    const value = handrailsEnv[key];
    if (!value) continue;
    envLines.push(`${key}=${value}`);
  }

  const issueIdHint = payload.taskId ?? payload.issueId ?? "";
  const apiBaseHint = handrailsEnv.HANDRAILS_API_URL ?? "<set HANDRAILS_API_URL>";

  const lines = [
    "Handrails wake event for a cloud adapter.",
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `HANDRAILS_API_KEY=<token from ${claimedApiKeyPath}>`,
    "",
    `Load HANDRAILS_API_KEY from ${claimedApiKeyPath} (the token you saved after claim-api-key).`,
    "",
    ...
    "",
    "HTTP rules:",
    "- Use Authorization: Bearer $HANDRAILS_API_KEY on every API call.",
    "- Use X-Handrails-Run-Id: $HANDRAILS_RUN_ID on every mutating API call.",
    "- Use only /api endpoints listed below.",
    "- Do NOT call guessed endpoints like /api/cloud-adapter/*, /api/cloud-adapters/*, /api/adapters/cloud/*, or /api/heartbeat.",
    "",
    "Workflow:",
    "1) GET /api/agents/me",
    `2) Determine issueId: HANDRAILS_TASK_ID if present, otherwise issue_id (${issueIdHint}).`,
    "3) If issueId exists:",
    "   - POST /api/issues/{issueId}/checkout with {\"agentId\":\"$HANDRAILS_AGENT_ID\",\"expectedStatuses\":[\"todo\",\"backlog\",\"blocked\"]}",
    ...
    "   - GET /api/companies/$HANDRAILS_COMPANY_ID/issues?assigneeAgentId=$HANDRAILS_AGENT_ID&status=todo,in_progress,blocked",
...
function buildStandardHandrailsPayload(
  ctx: AdapterExecutionContext,
  wakePayload: WakePayload,
  handrailsEnv: Record<string, string>,
  payloadTemplate: Record<string, unknown>,
): Record<string, unknown> {
  const templateHandrails = parseObject(payloadTemplate.handrails);
  const workspace = asRecord(ctx.context.handrailsWorkspace);
  const workspaces = Array.isArray(ctx.context.handrailsWorkspaces)
    ? ctx.context.handrailsWorkspaces.filter((entry): entry is Record<string, unknown> => Boolean(asRecord(entry)))
    : [];
  const configuredWorkspaceRuntime = parseObject(ctx.config.workspaceRuntime);
  const runtimeServiceIntents = Array.isArray(ctx.context.handrailsRuntimeServiceIntents)
    ? ctx.context.handrailsRuntimeServiceIntents.filter(
        (entry): entry is Record<string, unknown> => Boolean(asRecord(entry)),
      )
    : [];

  const standardHandrails: Record<string, unknown> = {
    runId: ctx.runId,
    ...
    apiUrl: handrailsEnv.HANDRAILS_API_URL ?? null,
  };

  if (workspace) {
    standardHandrails.workspace = workspace;
  }
  if (workspaces.length > 0) {
    standardHandrails.workspaces = workspaces;
  }
  if (runtimeServiceIntents.length > 0 || Object.keys(configuredWorkspaceRuntime).length > 0) {
    standardHandrails.workspaceRuntime = {
...
class GatewayWsClient {
  ...
  close() {
    if (!this.ws) return;
    this.ws.close(1000, "handrails-complete");
    this.ws = null;
  }
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  const handrailsEnv = buildHandrailsEnvForWake(ctx, wakePayload);
  const wakeText = buildWakeText(wakePayload, handrailsEnv);
  ...
  const handrailsPayload = buildStandardHandrailsPayload(ctx, wakePayload, handrailsEnv, payloadTemplate);
