...
import { asString, parseObject } from "@handrailsai/adapter-utils/server-utils";
    ...
    try {
      ...
      if (probeResult === "ok") {
        ...
        checks.push({
          ...
          hint: "Verify network reachability and gateway URL from the Handrails server host.",
        });
      }
    } catch (err) {
      checks.push({
