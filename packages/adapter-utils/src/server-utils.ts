...
const HANDRAILS_SKILL_ROOT_RELATIVE_CANDIDATES = [
  "../../skills",
  "../../../../../skills",
];

export interface HandrailsSkillEntry {
  key: string;
  runtimeName: string;
  source: string;
  required?: boolean;
...
interface PersistentSkillSnapshotOptions {
  adapterType: string;
  availableEntries: HandrailsSkillEntry[];
...
> {
  if (entry.required) {
    return {
      origin: "handrails_required",
      originLabel: "Required by Handrails",
      readOnly: false,
    };
  }
  return {
    origin: "company_managed",
    originLabel: "Managed by Handrails",
    readOnly: false,
  };
}
...
export function buildInvocationEnvForLogs(
  ...
  if (resolvedCommand) {
    merged[options.resolvedCommandEnvKey ?? "HANDRAILS_RESOLVED_COMMAND"] = resolvedCommand;
  }

  return redactEnvForLogs(merged);
}

export function buildHandrailsEnv(agent: { id: string; companyId: string }): Record<string, string> {
  ...
  const runtimeHost = resolveHostForUrl(
    process.env.HANDRAILS_LISTEN_HOST ?? process.env.HOST ?? "localhost",
  );
  const runtimePort = process.env.HANDRAILS_LISTEN_PORT ?? process.env.PORT ?? "3100";
  const apiUrl = process.env.HANDRAILS_API_URL ?? `http://${runtimeHost}:${runtimePort}`;
  vars.HANDRAILS_API_URL = apiUrl;
  return vars;
}
...
export async function resolveHandrailsSkillsDir(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<string | null> {
  const candidates = [
    ...HANDRAILS_SKILL_ROOT_RELATIVE_CANDIDATES.map((relativePath) => path.resolve(moduleDir, relativePath)),
...
export async function listHandrailsSkillEntries(
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<HandrailsSkillEntry[]> {
  const root = await resolveHandrailsSkillsDir(moduleDir, additionalCandidates);
  if (!root) return [];
  ...
  try {
    ...
    return entries
      ...
      .map((entry) => ({
        key: `handrailsai/handrails/${entry.name}`,
...
export function buildPersistentSkillSnapshot(
  ...
  for (const desiredSkill of desiredSkills) {
    ...
    warnings.push(`Desired skill "${desiredSkill}" is not available from the Handrails skills directory.`);
    entries.push({
      key: desiredSkill,
      runtimeName: null,
      desired: true,
      ...
      sourcePath: null,
      targetPath: null,
      detail: "Handrails cannot find this skill in the local runtime skills directory.",
...
function normalizeConfiguredHandrailsRuntimeSkills(value: unknown): HandrailsSkillEntry[] {
  if (!Array.isArray(value)) return [];
  const out: HandrailsSkillEntry[] = [];
...
export async function readHandrailsRuntimeSkillEntries(
  config: Record<string, unknown>,
  moduleDir: string,
  additionalCandidates: string[] = [],
): Promise<HandrailsSkillEntry[]> {
  const configuredEntries = normalizeConfiguredHandrailsRuntimeSkills(config.handrailsRuntimeSkills);
  if (configuredEntries.length > 0) return configuredEntries;
  return listHandrailsSkillEntries(moduleDir, additionalCandidates);
}

export async function readHandrailsSkillMarkdown(
  moduleDir: string,
  skillKey: string,
): Promise<string | null> {
  ...
  const entries = await listHandrailsSkillEntries(moduleDir);
...
export function readHandrailsSkillSyncPreference(config: Record<string, unknown>): {
  explicit: boolean;
  desiredSkills: string[];
} {
  const raw = config.handrailsSkillSync;
...
function canonicalizeDesiredHandrailsSkillReference(
  reference: string,
...
export function resolveHandrailsDesiredSkillNames(
  config: Record<string, unknown>,
  availableEntries: Array<{ key: string; runtimeName?: string | null; required?: boolean }>,
): string[] {
  const preference = readHandrailsSkillSyncPreference(config);
  ...
  const desiredSkills = preference.desiredSkills
    .map((reference) => canonicalizeDesiredHandrailsSkillReference(reference, availableEntries))
    .filter(Boolean);
  return Array.from(new Set([...requiredSkills, ...desiredSkills]));
}

export function writeHandrailsSkillSyncPreference(
  config: Record<string, unknown>,
  desiredSkills: string[],
): Record<string, unknown> {
  const next = { ...config };
  const raw = next.handrailsSkillSync;
  const current =
  ...
  next.handrailsSkillSync = current;
  return next;
}

export async function ensureHandrailsSkillSymlink(
  source: string,
  target: string,
...
export async function runChildProcess(
  ...
  return new Promise<RunProcessResult>((resolve, reject) => {
    ...
    // These vars leak in when the Handrails server itself is started from
    // within a Claude Code session (e.g. `npx handrailsai run` in a terminal
