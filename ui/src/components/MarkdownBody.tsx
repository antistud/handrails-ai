...
function MermaidDiagramBlock({ source, darkMode }: { source: string; darkMode: boolean }) {
  ...
  useEffect(() => {
    ...
    loadMermaid()
      .then(async (mermaid) => {
        ...
        const rendered = await mermaid.render(`handrails-mermaid-${renderId}`, source);
        if (!active) return;
        setSvg(rendered.svg);
      })
      .catch((err) => {
  ...
  return (
    <div className="handrails-mermaid">
      {svg ? (
        <div dangerouslySetInnerHTML={{ __html: svg }} />
      ) : (
        <>
          <p className={cn("handrails-mermaid-status", error && "handrails-mermaid-status-error")}>
            {error ? `Unable to render Mermaid diagram: ${error}` : "Rendering Mermaid diagram..."}
          </p>
          <pre className="handrails-mermaid-source">
...
export function MarkdownBody({ children, className, resolveImageSrc }: MarkdownBodyProps) {
        ...
        return (
          <a
            ...
            className={cn(
              ...
              `handrails-mention-chip--${parsed.kind}`,
              parsed.kind === "project" && "handrails-project-mention-chip",
            )}
  ...
  return (
    <div
      className={cn(
        "handrails-markdown prose prose-sm max-w-none break-words overflow-hidden",
