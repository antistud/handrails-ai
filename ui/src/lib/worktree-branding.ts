...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("handrails-worktree-enabled") !== "true") return null;

  const name = readMetaContent("handrails-worktree-name");
  const color = normalizeHexColor(readMetaContent("handrails-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("handrails-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
