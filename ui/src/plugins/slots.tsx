 ...
 *
 * Plugin bundles are built with `external: ["@handrailsai/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__handrailsPluginBridge__`)
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    case "react":
      source = `
        const R = globalThis.__handrailsPluginBridge__?.react;
        export default R;
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__handrailsPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__handrailsPluginBridge__?.sdkUi ?? {};
 ...
 * - `import { ... } from "@handrailsai/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@handrailsai/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@handrailsai/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@handrailsai/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@handrailsai/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__handrailsPluginBridge__) {
 ...
 * self-register via `window.handrailsPlugins.registerReactComponent()`.
 ...
 * of `@handrailsai/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
