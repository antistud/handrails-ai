...
/**
 * Filter .handrails.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterHandrailsYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://handrails.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm handrailsai company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [Handrails](https://handrails.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [Handrails](https://handrails.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .handrails.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .handrails.yaml
    const yamlPath = exportData.handrailsExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterHandrailsYaml(exportData.files[yamlPath], checkedFiles);
    }
