...
import type { HandrailsConfig } from "@handrailsai/shared";
...
function formatEnvEntries(entries: Record<string, string>): string {
  return [
    ...
    "# Generated by Handrails worktree repair",
...
function resolveWorktreeRuntimeContext(
  ...
  if (env.HANDRAILS_IN_WORKTREE !== "true") return null;

  const configPath = resolveHandrailsConfigPath(overrideConfigPath);
  const envPath = resolveHandrailsEnvPath(configPath);
  const worktreeRoot = path.resolve(path.dirname(configPath), "..");
  const worktreeName = nonEmpty(env.HANDRAILS_WORKTREE_NAME) ?? path.basename(worktreeRoot);
  const instanceId = nonEmpty(env.HANDRAILS_INSTANCE_ID) ?? sanitizeWorktreeInstanceId(worktreeName);
  const homeDir = resolveHomeAwarePath(
    nonEmpty(env.HANDRAILS_HOME) ??
      nonEmpty(env.HANDRAILS_WORKTREES_DIR) ??
      "~/.handrails-worktrees",
  );
...
function writeConfigFile(configPath: string, config: HandrailsConfig): void {
...
function collectSiblingWorktreePorts(context: WorktreeRuntimeContext): {
  ...
  if (repoManagedWorktreesRoot && fs.existsSync(repoManagedWorktreesRoot)) {
    for (const entry of fs.readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const siblingConfigPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".handrails", "config.json");
  ...
  for (const siblingConfigPath of siblingConfigPaths) {
    try {
      const siblingConfig = JSON.parse(fs.readFileSync(siblingConfigPath, "utf8")) as HandrailsConfig;
...
function buildIsolatedWorktreeConfig(
  ...
  const databasePort =
    config.database.mode === "embedded-postgres"
      ...
      : undefined;
  const nextConfig: HandrailsConfig = {
    ...config,
    database: {
      ...config.database,
      ...(config.database.mode === "embedded-postgres"
...
): { config: HandrailsConfig; changed: boolean } {
  let changed = false;
...
export function maybeRepairLegacyWorktreeConfigAndEnvFiles(): {
  ...
  process.env.HANDRAILS_HOME = context.homeDir;
  process.env.HANDRAILS_INSTANCE_ID = context.instanceId;
  process.env.HANDRAILS_CONFIG = context.configPath;
  process.env.HANDRAILS_CONTEXT = context.contextPath;
  process.env.HANDRAILS_WORKTREE_NAME = context.worktreeName;

  let repairedConfig = false;
  if (fs.existsSync(context.configPath)) {
    try {
      const parsed = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as HandrailsConfig;
  ...
  const desiredEnvEntries: Record<string, string> = {
    ...existingEnvEntries,
    HANDRAILS_HOME: context.homeDir,
    HANDRAILS_INSTANCE_ID: context.instanceId,
    HANDRAILS_CONFIG: context.configPath,
    HANDRAILS_CONTEXT: context.contextPath,
    HANDRAILS_IN_WORKTREE: "true",
    HANDRAILS_WORKTREE_NAME: context.worktreeName,
  };
...
export function maybePersistWorktreeRuntimePorts(input: {
  ...
  let fileConfig: HandrailsConfig;
  try {
    fileConfig = JSON.parse(fs.readFileSync(context.configPath, "utf8")) as HandrailsConfig;
  } catch {
    return;
  }
