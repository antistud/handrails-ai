...
function jwtConfig() {
  const secret = process.env.HANDRAILS_AGENT_JWT_SECRET;
  if (!secret) return null;

  return {
    secret,
    ttlSeconds: parseNumber(process.env.HANDRAILS_AGENT_JWT_TTL_SECONDS, 60 * 60 * 48),
    issuer: process.env.HANDRAILS_AGENT_JWT_ISSUER ?? "handrails",
    audience: process.env.HANDRAILS_AGENT_JWT_AUDIENCE ?? "handrails-api",
  };
}
