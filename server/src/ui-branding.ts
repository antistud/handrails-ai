const FAVICON_BLOCK_START = "<!-- HANDRAILS_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- HANDRAILS_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- HANDRAILS_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- HANDRAILS_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue(env.HANDRAILS_IN_WORKTREE);
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty(env.HANDRAILS_WORKTREE_NAME) ?? nonEmpty(env.HANDRAILS_INSTANCE_ID) ?? "worktree";
  const color = normalizeHexColor(env.HANDRAILS_WORKTREE_COLOR) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="handrails-worktree-enabled" content="true" />',
    `<meta name="handrails-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="handrails-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="handrails-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
