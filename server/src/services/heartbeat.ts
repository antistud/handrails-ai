...
import type { BillingType, ExecutionWorkspaceConfig } from "@handrailsai/shared";
import {
  agents,
  agentRuntimeState,
  agentTaskSessions,
...
const DEFERRED_WAKE_CONTEXT_KEY = "_handrailsWakeContext";
const DETACHED_PROCESS_ERROR_CODE = "process_detached";
const startLocksByAgent = new Map<string, Promise<void>>();
const REPO_ONLY_CWD_SENTINEL = "/__handrails_repo_only__";
...
export function heartbeatService(db: Db) {
  ...
  async function executeRun(runId: string) {
    ...
    context.handrailsWorkspaces = resolvedWorkspace.workspaceHints;
    ...
    if (runtimeServiceIntents.length > 0) {
      context.handrailsRuntimeServiceIntents = runtimeServiceIntents;
    } else {
      delete context.handrailsRuntimeServiceIntents;
    }
    ...
    if (sessionCompaction.rotate) {
      context.handrailsSessionHandoffMarkdown = sessionCompaction.handoffMarkdown;
      context.handrailsSessionRotationReason = sessionCompaction.reason;
      context.handrailsPreviousSessionId = previousSessionDisplayId ?? runtimeSessionIdForAdapter;
      ...
      delete context.handrailsSessionHandoffMarkdown;
      delete context.handrailsSessionRotationReason;
      delete context.handrailsPreviousSessionId;
    }
    ...
    try {
      ...
      if (runtimeServices.length > 0) {
        context.handrailsRuntimeServices = runtimeServices;
        context.handrailsRuntimePrimaryUrl =
      ...
      if (issueId && (executionWorkspace.created || runtimeServices.some((service) => !service.reused))) {
        try {
          ...
          await onLog(
            "stderr",
            `[handrails] Failed to post workspace-ready comment: ${err instanceof Error ? err.message : String(err)}\n`,
          );
        }
      }
      ...
      if (adapter.supportsLocalAgentJwt && !authToken) {
        logger.warn(
          ...
          "local agent jwt secret missing or invalid; running without injected HANDRAILS_API_KEY",
        );
      }
      ...
      if (adapterManagedRuntimeServices.length > 0) {
        ...
        context.handrailsRuntimeServices = combinedRuntimeServices;
        context.handrailsRuntimePrimaryUrl =
        ...
        if (issueId) {
          try {
            ...
            await onLog(
              "stderr",
              `[handrails] Failed to post adapter-managed runtime comment: ${err instanceof Error ? err.message : String(err)}\n`,
            );
          }
        }
      }
