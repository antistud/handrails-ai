...
import {
  readHandrailsSkillSyncPreference,
  writeHandrailsSkillSyncPreference,
} from "@handrailsai/adapter-utils/server-utils";
...
function classifyPortableFileKind(pathValue: string): CompanyPortabilityExportPreviewResult["fileInventory"][number]["kind"] {
  ...
  if (normalized === ".handrails.yaml" || normalized === ".handrails.yml") return "extension";
...
function readSkillKey(frontmatter: Record<string, unknown>) {
  ...
  const handrails = isPlainRecord(metadata?.handrails) ? metadata?.handrails as Record<string, unknown> : null;
  return normalizeSkillKey(
    asString(frontmatter.key)
    ?? asString(frontmatter.skillKey)
    ?? asString(metadata?.skillKey)
    ?? asString(metadata?.canonicalKey)
    ?? asString(metadata?.handrailsSkillKey)
    ?? asString(handrails?.skillKey)
    ?? asString(handrails?.key),
  );
}
...
function deriveManifestSkillKey(
  ...
  if (sourceKind === "handrails_bundled") {
    return `handrailsai/handrails/${slug}`;
  }
...
const ADAPTER_DEFAULT_RULES_BY_TYPE: Record<string, Array<{ path: string[]; value: unknown }>> = {
  ...
  openclaw_gateway: [
    ...
    { path: ["sessionKey"], value: "handrails" },
...
function buildLegacyRoutineTriggerFromRecurrence(
  ...
  if (!frequency) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence without frequency; add .handrails.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  if (interval < 1) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid interval; add .handrails.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (startsAt && !zonedStartsAt) {
    errors.push(`Recurring task ${issue.slug} has an invalid legacy startsAt/timezone combination; add .handrails.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
  ...
  if (hour < 0 || hour > 23 || minute < 0 || minute > 59) {
    errors.push(`Recurring task ${issue.slug} uses legacy recurrence with an invalid time; add .handrails.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }

  if (issue.legacyRecurrence.until != null || issue.legacyRecurrence.count != null) {
    warnings.push(`Recurring task ${issue.slug} uses legacy recurrence end bounds; Handrails will import the routine trigger without those limits.`);
  }
  ...
  if (frequency === "hourly") {
    ...
    if (Array.isArray(issue.legacyRecurrence.weekdays) || Array.isArray(issue.legacyRecurrence.monthDays) || Array.isArray(issue.legacyRecurrence.months)) {
      errors.push(`Recurring task ${issue.slug} uses unsupported legacy daily recurrence constraints; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence with interval > 1; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (cronWeekdays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy weekly recurrence without weekdays; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} * * ${normalizeCronList(cronWeekdays)}`;
  } else if (frequency === "monthly") {
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence with interval > 1; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    if (Array.isArray(issue.legacyRecurrence.ordinalWeekdays) && issue.legacyRecurrence.ordinalWeekdays.length > 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy ordinal monthly recurrence; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy monthly recurrence without monthDays; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    ...
    if (interval !== 1) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence with interval > 1; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    const months = Array.isArray(issue.legacyRecurrence.months)
      ? issue.legacyRecurrence.months
    ...
    if (months.length === 0 || monthDays.length === 0) {
      errors.push(`Recurring task ${issue.slug} uses legacy yearly recurrence without month/monthDay anchors; add .handrails.yaml routines.${issue.slug}.triggers.`);
      return { trigger: null, warnings, errors };
    }
    cronExpression = `${minute} ${hour} ${normalizeCronList(monthDays.map(String))} ${normalizeCronList(months.map(String))} *`;
  } else {
    errors.push(`Recurring task ${issue.slug} uses unsupported legacy recurrence frequency "${frequency}"; add .handrails.yaml routines.${issue.slug}.triggers.`);
    return { trigger: null, warnings, errors };
  }
...
function filterExportFiles(
  ...
  const extensionEntry = filtered[handrailsExtensionPath];
  if (selectedFiles.has(handrailsExtensionPath) && typeof extensionEntry === "string") {
    filtered[handrailsExtensionPath] = filterPortableExtensionYaml(extensionEntry, selectedFiles);
  }

  return filtered;
}

function findHandrailsExtensionPath(files: Record<string, CompanyPortabilityFileEntry>) {
  if (typeof files[".handrails.yaml"] === "string") return ".handrails.yaml";
  if (typeof files[".handrails.yml"] === "string") return ".handrails.yml";
  return Object.keys(files).find((entry) => entry.endsWith("/.handrails.yaml") || entry.endsWith("/.handrails.yml")) ?? null;
}
...
async function buildSkillSourceEntry(skill: CompanySkill) {
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "handrails_bundled") {
    const commit = await resolveBundledSkillsCommit();
    return {
      kind: "github-dir",
      repo: "handrailsai/handrails",
      path: `skills/${skill.slug}`,
      commit,
      trackingRef: "master",
      url: `https://github.com/handrailsai/handrails/tree/master/skills/${skill.slug}`,
    };
  }
...
function shouldReferenceSkillOnExport(skill: CompanySkill, expandReferencedSkills: boolean) {
  ...
  const metadata = isPlainRecord(skill.metadata) ? skill.metadata : null;
  if (asString(metadata?.sourceKind) === "handrails_bundled") return true;
...
async function withSkillSourceMetadata(skill: CompanySkill, markdown: string) {
  ...
  metadata.handrailsSkillKey = skill.key;
  metadata.handrails = {
    ...(isPlainRecord(metadata.handrails) ? metadata.handrails : {}),
    skillKey: skill.key,
    slug: skill.slug,
  };
  const frontmatter = {
...
function buildManifestFromPackageFiles(
  ...
  const handrailsExtensionPath = findHandrailsExtensionPath(normalizedFiles);
  const handrailsExtension = handrailsExtensionPath
    ? parseYamlFile(readPortableTextFile(normalizedFiles, handrailsExtensionPath) ?? "")
    : {};
  const handrailsCompany = isPlainRecord(handrailsExtension.company) ? handrailsExtension.company : {};
  const handrailsSidebar = normalizePortableSidebarOrder(handrailsExtension.sidebar);
  const handrailsAgents = isPlainRecord(handrailsExtension.agents) ? handrailsExtension.agents : {};
  const handrailsProjects = isPlainRecord(handrailsExtension.projects) ? handrailsExtension.projects : {};
  const handrailsTasks = isPlainRecord(handrailsExtension.tasks) ? handrailsExtension.tasks : {};
  const handrailsRoutines = isPlainRecord(handrailsExtension.routines) ? handrailsExtension.routines : {};
  const companyName =
  ...
  const manifest: CompanyPortabilityManifest = {
    ...
    company: {
      ...
      brandColor: asString(handrailsCompany.brandColor),
      logoPath: asString(handrailsCompany.logoPath) ?? asString(handrailsCompany.logo),
      requireBoardApprovalForNewAgents:
        typeof handrailsCompany.requireBoardApprovalForNewAgents === "boolean"
          ? handrailsCompany.requireBoardApprovalForNewAgents
          : readCompanyApprovalDefault(companyFrontmatter),
    },
    sidebar: handrailsSidebar,
    agents: [],
    skills: [],
    projects: [],
    issues: [],
  ...
  for (const agentPath of agentPaths) {
    ...
    const extension = isPlainRecord(handrailsAgents[slug]) ? handrailsAgents[slug] : {};
  ...
  for (const projectPath of projectPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(handrailsProjects[slug]) ? handrailsProjects[slug] : {};
  ...
  for (const taskPath of taskPaths) {
    ...
    const slug = asString(frontmatter.slug) ?? fallbackSlug;
    const extension = isPlainRecord(handrailsTasks[slug]) ? handrailsTasks[slug] : {};
    const routineExtension = normalizeRoutineExtension(handrailsRoutines[slug]);
    const routineExtensionRaw = isPlainRecord(handrailsRoutines[slug]) ? handrailsRoutines[slug] : {};
...
export function companyPortabilityService(db: Db, storage?: StorageService) {
    ...
    const candidatePaths = (tree.tree ?? [])
      ...
      .filter((entry) => {
        ...
        return (
          ...
          relative === ".handrails.yaml" ||
          relative === ".handrails.yml"
        );
      });
  ...
  async function exportBundle(
    ...
    const handrailsAgentsOut: Record<string, Record<string, unknown>> = {};
    const handrailsProjectsOut: Record<string, Record<string, unknown>> = {};
    const handrailsTasksOut: Record<string, Record<string, unknown>> = {};
    const unportableTaskWorkspaceRefs = new Map<string, { workspaceId: string; taskSlugs: string[] }>();
    const handrailsRoutinesOut: Record<string, Record<string, unknown>> = {};
    ...
    if (include.agents) {
      for (const agent of agentRows) {
        ...
        const desiredSkills = readHandrailsSkillSyncPreference(
        ...
        handrailsAgentsOut[slug] = isPlainRecord(extension) ? extension : {};
      }
    }
    ...
    for (const project of selectedProjectRows) {
      ...
      handrailsProjectsOut[slug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const issue of selectedIssueRows) {
      ...
      handrailsTasksOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }
    ...
    for (const routine of selectedRoutineRows) {
      ...
      handrailsRoutinesOut[taskSlug] = isPlainRecord(extension) ? extension : {};
    }

    const handrailsExtensionPath = ".handrails.yaml";
    const handrailsAgents = Object.fromEntries(
      Object.entries(handrailsAgentsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const handrailsProjects = Object.fromEntries(
      Object.entries(handrailsProjectsOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const handrailsTasks = Object.fromEntries(
      Object.entries(handrailsTasksOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    const handrailsRoutines = Object.fromEntries(
      Object.entries(handrailsRoutinesOut).filter(([, value]) => isPlainRecord(value) && Object.keys(value).length > 0),
    );
    files[handrailsExtensionPath] = buildYamlFile(
      {
        schema: "handrails/v1",
        ...
        agents: Object.keys(handrailsAgents).length > 0 ? handrailsAgents : undefined,
        projects: Object.keys(handrailsProjects).length > 0 ? handrailsProjects : undefined,
        tasks: Object.keys(handrailsTasks).length > 0 ? handrailsTasks : undefined,
        routines: Object.keys(handrailsRoutines).length > 0 ? handrailsRoutines : undefined,
      },
      { preserveEmptyStrings: true },
    );

    let finalFiles = filterExportFiles(files, input.selectedFiles, handrailsExtensionPath);
  ...
  async function importBundle(
    ...
    if (include.agents) {
      for (const planAgent of plan.preview.plan.agentPlans) {
        ...
        const adapterConfigWithSkills = writeHandrailsSkillSyncPreference(
          baseAdapterConfig,
          desiredSkills,
        );
