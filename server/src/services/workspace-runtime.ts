...
import type { AdapterRuntimeServiceReport } from "@handrailsai/adapter-utils";
import type { Db } from "@handrailsai/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@handrailsai/db";
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.HANDRAILS_WORKSPACE_CWD = input.worktreePath;
  env.HANDRAILS_WORKSPACE_PATH = input.worktreePath;
  env.HANDRAILS_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.HANDRAILS_WORKSPACE_BRANCH = input.branchName;
  env.HANDRAILS_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.HANDRAILS_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.HANDRAILS_WORKSPACE_SOURCE = input.base.source;
  env.HANDRAILS_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.HANDRAILS_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.HANDRAILS_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.HANDRAILS_PROJECT_ID = input.base.projectId ?? "";
  env.HANDRAILS_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.HANDRAILS_AGENT_ID = input.agent.id ?? "";
  env.HANDRAILS_AGENT_NAME = input.agent.name;
  env.HANDRAILS_COMPANY_ID = input.agent.companyId;
  env.HANDRAILS_ISSUE_ID = input.issue?.id ?? "";
  env.HANDRAILS_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.HANDRAILS_ISSUE_TITLE = input.issue?.title ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.HANDRAILS_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.HANDRAILS_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.HANDRAILS_WORKSPACE_WORKTREE_PATH =
    input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.HANDRAILS_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.HANDRAILS_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.HANDRAILS_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.HANDRAILS_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.HANDRAILS_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.HANDRAILS_PROJECT_ID = input.workspace.projectId ?? "";
  env.HANDRAILS_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.HANDRAILS_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".handrails", "worktrees");
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Handrails", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
