...
import { pluginLogs, agentTaskSessions as agentTaskSessionsTable } from "@handrailsai/db";
 ...
 *
 * This implementation delegates to the core Handrails domain services,
  ...
  return {
    ...
    events: {
      ...
      async subscribe(params: { eventPattern: string; filter?: Record<string, unknown> | null }) {
        const handler = async (event: import("@handrailsai/plugin-sdk").PluginEvent) => {
          if (notifyWorker) {
