 ...
 *    (`~/.handrails/plugins/`) and `node_modules` for packages matching
 *    the `handrails-plugin-*` naming convention. Aggregates results with
...
/**
 * Naming convention for npm-published Handrails plugins.
 * Packages matching this pattern are considered Handrails plugins.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export const NPM_PLUGIN_PACKAGE_PREFIX = "handrails-plugin-";
...
export interface DiscoveredPlugin {
  ...
  manifest: HandrailsPluginManifestV1 | null;
}
...
export type PluginSource =
  | "local-filesystem"  // ~/.handrails/plugins/ local directory
  | "npm"               // npm packages matching handrails-plugin-* convention
...
function getDeclaredPageRoutePaths(manifest: HandrailsPluginManifestV1): string[] {
...
export interface PluginLoaderOptions {
  ...
  /**
   * Whether to discover installed npm packages matching the handrails-plugin-*
   * naming convention.
   * Defaults to true.
   */
...
export interface PluginInstallOptions {
  /**
   * npm package name to install (e.g. "handrails-plugin-linear" or "@acme/plugin-linear").
...
export interface PluginRuntimeServices {
   ...
   */
  buildHostHandlers: (pluginId: string, manifest: HandrailsPluginManifestV1) => WorkerToHostHandlers;
  /**
...
export interface PluginLoader {
  ...
  /**
   * Discover Handrails plugins installed as npm packages in the current
   * Node.js environment matching the "handrails-plugin-*" naming convention.
   *
   ...
   * the "handrailsPlugin.manifest" key, loads the manifest module, and
   * validates it against the plugin manifest schema.
   *
   * Returns null if the package is not a Handrails plugin.
   * Throws if the package is a Handrails plugin but the manifest is invalid.
   *
   * @see PLUGIN_SPEC.md §10 — Package Contract
   */
  loadManifest(packagePath: string): Promise<HandrailsPluginManifestV1 | null>;
  ...
  upgradePlugin(pluginId: string, options: Omit<PluginInstallOptions, "installDir">): Promise<{
    oldManifest: HandrailsPluginManifestV1;
    newManifest: HandrailsPluginManifestV1;
    discovered: DiscoveredPlugin;
  }>;
...
/**
 * Check whether a package name matches the Handrails plugin naming convention.
 * Accepts both the "handrails-plugin-" prefix and scoped "@scope/plugin-" packages.
 *
 * @see PLUGIN_SPEC.md §10 — Package Contract
 */
export function isPluginPackageName(name: string): boolean {
  if (name.startsWith(NPM_PLUGIN_PACKAGE_PREFIX)) return true;
  // Also accept scoped packages like @acme/plugin-linear or @handrailsai/plugin-*
  if (name.includes("/")) {
 ...
 *
 * The spec defines a "handrailsPlugin" key in package.json with a "manifest"
...
function resolveManifestPath(
  ...
  const handrailsPlugin = pkgJson["handrailsPlugin"];
  if (
    handrailsPlugin !== null &&
    typeof handrailsPlugin === "object" &&
    !Array.isArray(handrailsPlugin)
  ) {
    const manifestRelPath = (handrailsPlugin as Record<string, unknown>)[
      "manifest"
    ];
...
function getMinimumHostVersion(manifest: HandrailsPluginManifestV1): string | undefined {
  return manifest.minimumHostVersion ?? manifest.minimumHandrailsVersion;
}
 ...
 *   packageName: "handrails-plugin-linear",
 *   version: "^1.0.0",
 * });
 * ```
 *
...
export function pluginLoader(
  ...
  async function assertPageRoutePathsAvailable(manifest: HandrailsPluginManifestV1): Promise<void> {
    ...
    for (const plugin of installedPlugins) {
      ...
      const installedManifest = plugin.manifestJson as HandrailsPluginManifestV1 | null;
  ...
  async function fetchAndValidate(
    ...
    if (!manifestPath || !existsSync(manifestPath)) {
      throw new Error(
        `Package ${resolvedPackageName} at ${resolvedPackagePath} does not appear to be a Handrails plugin (no manifest found).`,
      );
    }
  ...
  async function loadManifestFromPath(
    manifestPath: string,
  ): Promise<HandrailsPluginManifestV1> {
    let raw: unknown;
   ...
   * if the package is not a Handrails plugin.
   */
  async function buildDiscoveredPlugin(
    ...
    const hasHandrailsPlugin = "handrailsPlugin" in pkgJson;
    const nameMatchesConvention = isPluginPackageName(packageName);

    if (!hasHandrailsPlugin && !nameMatchesConvention) {
      return null;
    }
  ...
  return {
    ...
    async loadManifest(packagePath: string): Promise<HandrailsPluginManifestV1 | null> {
      const pkgJson = await readPackageJson(packagePath);
      if (!pkgJson) return null;
    ...
    async upgradePlugin(
      ...
      const plugin = (await registry.getById(pluginId)) as {
        ...
        manifestJson: HandrailsPluginManifestV1;
      } | null;
  ...
  async function activatePlugin(plugin: PluginRecord): Promise<PluginLoadResult> {
    ...
    try {
      ...
      // (for example @handrailsai/shared exports). Run those workers through
