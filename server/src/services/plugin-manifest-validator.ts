 ...
 */
import { pluginManifestV1Schema } from "@handrailsai/shared";
import type { HandrailsPluginManifestV1 } from "@handrailsai/shared";
import { PLUGIN_API_VERSION } from "@handrailsai/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): HandrailsPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as HandrailsPluginManifestV1,
        };
      }
