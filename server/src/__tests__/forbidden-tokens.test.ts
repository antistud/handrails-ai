...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "handrails", LOGNAME: "handrails", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "handrails" }),
      },
    );

    expect(tokens).toEqual(["handrails", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "handrails" }, {
        userInfo: () => ({ username: "handrails" }),
      });

      expect(tokens).toEqual(["handrails", "custom-token"]);
    } finally {
