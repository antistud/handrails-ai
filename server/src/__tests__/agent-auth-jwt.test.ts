...
describe("agent local JWT", () => {
  const secretEnv = "HANDRAILS_AGENT_JWT_SECRET";
  const ttlEnv = "HANDRAILS_AGENT_JWT_TTL_SECONDS";
  const issuerEnv = "HANDRAILS_AGENT_JWT_ISSUER";
  const audienceEnv = "HANDRAILS_AGENT_JWT_AUDIENCE";
  ...
  it("rejects issuer/audience mismatch", () => {
    ...
    process.env[issuerEnv] = "handrails";
    process.env[audienceEnv] = "handrails-api";
    expect(verifyLocalAgentJwt(token!)).toBeNull();
  });
});
