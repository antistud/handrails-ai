...
import type { CompanyPortabilityFileEntry } from "@handrailsai/shared";
...
describe("company portability", () => {
    ...
    const companySkills = [
      {
        ...
        slug: "handrails",
        name: "handrails",
        description: "Handrails coordination skill",
        markdown: "---\nname: handrails\ndescription: Handrails coordination skill\n---\n\n# Handrails\n",
        sourceType: "github",
        sourceLocator: "https://github.com/handrailsai/handrails/tree/master/skills/handrails",
  ...
  it("parses canonical GitHub import URLs with explicit ref and package path", () => {
    expect(
      parseGitHubSourceUrl("https://github.com/handrailsai/companies?ref=feature%2Fdemo&path=gstack"),
    ).toEqual({
      owner: "handrailsai",
      repo: "companies",
      ref: "feature/demo",
  ...
  it("parses canonical GitHub import URLs with explicit companyPath", () => {
    expect(
      parseGitHubSourceUrl(
        "https://github.com/handrailsai/companies?ref=abc123&companyPath=gstack%2FCOMPANY.md",
      ),
    ).toEqual({
      owner: "handrailsai",
      repo: "companies",
  ...
  it("exports referenced skills as stubs by default with sanitized Handrails extension data", async () => {
    ...
    expect(asTextFile(exported.files["COMPANY.md"])).toContain('name: "Handrails"');
    ...
    expect(asTextFile(exported.files["agents/claudecoder/AGENTS.md"])).toContain(`- "${handrailsKey}"`);
    expect(asTextFile(exported.files["agents/cmo/AGENTS.md"])).not.toContain("skills:");
    expect(asTextFile(exported.files["skills/handrailsai/handrails/handrails/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/handrailsai/handrails/handrails/SKILL.md"])).toContain('kind: "github-dir"');
    expect(exported.files["skills/handrailsai/handrails/handrails/references/api.md"]).toBeUndefined();
    ...
    const extension = asTextFile(exported.files[".handrails.yaml"]);
    expect(extension).toContain('schema: "handrails/v1"');
    ...
    expect(extension).not.toContain("handrailsSkillSync");
  ...
  it("exports default sidebar order into the Handrails extension and manifest", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(asTextFile(exported.files[".handrails.yaml"])).toContain([
      "sidebar:",
      "  agents:",
      '    - "claudecoder"',
      '    - "cmo"',
  ...
  it("expands referenced skills when requested", async () => {
    ...
    expect(asTextFile(exported.files["skills/handrailsai/handrails/handrails/SKILL.md"])).toContain("# Handrails");
    expect(asTextFile(exported.files["skills/handrailsai/handrails/handrails/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/handrailsai/handrails/handrails/references/api.md"])).toContain("# API");
  });
  ...
  it("exports only selected skills when skills filter is provided", async () => {
    ...
    expect(asTextFile(exported.files["skills/company/PAP/company-playbook/SKILL.md"])).toContain("# Company Playbook");
    expect(exported.files["skills/handrailsai/handrails/handrails/SKILL.md"]).toBeUndefined();
  });
  ...
  it("warns and exports all skills when skills filter matches nothing", async () => {
    ...
    expect(exported.files["skills/company/PAP/company-playbook/SKILL.md"]).toBeDefined();
    expect(exported.files["skills/handrailsai/handrails/handrails/SKILL.md"]).toBeDefined();
  });

  it("exports the company logo into images/ and references it from .handrails.yaml", async () => {
    const storage = {
    ...
    expect(exported.files[".handrails.yaml"]).toContain('logoPath: "images/company-logo.png"');
  });
  ...
  it("exports duplicate skill slugs into readable namespaced paths", async () => {
    ...
    companySkillSvc.listFull.mockResolvedValue([
      ...
      },
      {
        id: "skill-handrails",
        companyId: "company-1",
        key: "handrailsai/handrails/release-changelog",
        slug: "release-changelog",
        name: "release-changelog",
        description: "Bundled release changelog skill",
        markdown: "---\nname: release-changelog\n---\n\n# Bundled Release Changelog\n",
        sourceType: "github",
        sourceLocator: "https://github.com/handrailsai/handrails/tree/master/skills/release-changelog",
        sourceRef: "0123456789abcdef0123456789abcdef01234567",
        trustLevel: "markdown_only",
        compatibility: "compatible",
        fileInventory: [{ path: "SKILL.md", kind: "skill" }],
        metadata: {
          sourceKind: "handrails_bundled",
          owner: "handrailsai",
          repo: "handrails",
          ref: "0123456789abcdef0123456789abcdef01234567",
          trackingRef: "master",
    ...
    expect(asTextFile(exported.files["skills/handrailsai/handrails/release-changelog/SKILL.md"])).toContain("metadata:");
    expect(asTextFile(exported.files["skills/handrailsai/handrails/release-changelog/SKILL.md"])).toContain("handrailsai/handrails/release-changelog");
  });
  ...
  it("exports portable project workspace metadata and remaps it on import", async () => {
        ...
        workspaces: [
          {
            ...
            name: "Main Repo",
            sourceType: "git_repo",
            cwd: "/Users/dotta/handrails",
            repoUrl: "https://github.com/handrailsai/handrails.git",
            repoRef: "main",
            defaultRef: "main",
            visibility: "default",
            setupCommand: "pnpm install",
            cleanupCommand: "rm -rf .handrails-tmp",
            remoteProvider: null,
    ...
    expect(extension).toContain('repoUrl: "https://github.com/handrailsai/handrails.git"');
    expect(extension).toContain('defaultProjectWorkspaceKey: "main-repo"');
    expect(extension).toContain('projectWorkspaceKey: "main-repo"');
    expect(extension).not.toContain("/Users/dotta/handrails");
    ...
    await portability.importBundle({
      ...
      target: {
        mode: "new_company",
        newCompanyName: "Imported Handrails",
      },
  ...
  it("infers portable git metadata from a local checkout without task warning fan-out", async () => {
    const portability = companyPortabilityService({} as any);
    const repoDir = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-portability-git-"));
    execFileSync("git", ["init"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["checkout", "-b", "main"], { cwd: repoDir, stdio: "ignore" });
    execFileSync("git", ["remote", "add", "origin", "https://github.com/handrailsai/handrails.git"], {
      cwd: repoDir,
      stdio: "ignore",
    });
    ...
    const extension = asTextFile(exported.files[".handrails.yaml"]);
    expect(extension).toContain('repoUrl: "https://github.com/handrailsai/handrails.git"');
    expect(extension).toContain('projectWorkspaceKey: "handrails"');
  ...
  it("reads env inputs back from .handrails.yaml during preview import", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("exports routines as recurring task packages with Handrails routine extensions", async () => {
    const portability = companyPortabilityService({} as any);
  ...
  it("imports recurring task packages as routines instead of one-time issues", async () => {
    ...
    const preview = await portability.previewImport({
      source: { type: "inline", rootPath: "handrails-demo", files },
      include: { company: true, agents: true, projects: true, issues: true, skills: false },
      target: { mode: "new_company", newCompanyName: "Imported Handrails" },
      agents: "all",
      collisionStrategy: "rename",
    });
  ...
  it("migrates legacy schedule.recurrence imports into routine triggers", async () => {
    ...
    const files = {
      "COMPANY.md": ['---', 'schema: "agentcompanies/v1"', 'name: "Imported Handrails"', "---", ""].join("\n"),
  ...
  it("imports a vendor-neutral package without .handrails.yaml", async () => {
    const portability = companyPortabilityService({} as any);
    ...
    expect(preview.errors).toEqual([]);
    expect(preview.manifest.company?.name).toBe("Imported Handrails");
  ...
  it("imports a packaged company logo and attaches it to the target company", async () => {
    ...
    exported.files[".handrails.yaml"] = `${exported.files[".handrails.yaml"]}`.replace(
      'brandColor: "#5c5fff"\n',
