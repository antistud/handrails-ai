...
describe("agent instructions service", () => {
  const originalHandrailsHome = process.env.HANDRAILS_HOME;
  const originalHandrailsInstanceId = process.env.HANDRAILS_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalHandrailsHome === undefined) delete process.env.HANDRAILS_HOME;
    else process.env.HANDRAILS_HOME = originalHandrailsHome;
    if (originalHandrailsInstanceId === undefined) delete process.env.HANDRAILS_INSTANCE_ID;
    else process.env.HANDRAILS_INSTANCE_ID = originalHandrailsInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const handrailsHome = await makeTempDir("handrails-agent-instructions-home-");
    const externalRoot = await makeTempDir("handrails-agent-instructions-external-");
    cleanupDirs.add(handrailsHome);
    cleanupDirs.add(externalRoot);
    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("handrails-agent-instructions-new-external-");
    cleanupDirs.add(handrailsHome);
    cleanupDirs.add(externalRoot);
    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("handrails-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const handrailsHome = await makeTempDir("handrails-agent-instructions-recover-");
    cleanupDirs.add(handrailsHome);
    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const handrailsHome = await makeTempDir("handrails-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("handrails-agent-instructions-stale-root-");
    cleanupDirs.add(handrailsHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const handrailsHome = await makeTempDir("handrails-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("handrails-agent-instructions-heal-write-stale-");
    cleanupDirs.add(handrailsHome);
    cleanupDirs.add(staleRoot);
    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const handrailsHome = await makeTempDir("handrails-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("handrails-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(handrailsHome);
    cleanupDirs.add(staleRoot);
    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const handrailsHome = await makeTempDir("handrails-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("handrails-agent-instructions-partial-root-");
    cleanupDirs.add(handrailsHome);
    cleanupDirs.add(staleRoot);
    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = "test-instance";
