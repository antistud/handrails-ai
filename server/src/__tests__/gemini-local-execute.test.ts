...
import { execute } from "@handrailsai/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects handrails env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the handrails heartbeat.");
      expect(promptArg).toContain("Handrails runtime note:");
      ...
      expect(invocationPrompt).toContain("Handrails API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      await execute({
        ...
        config: {
          command: commandPath,
          cwd: workspace,
          env: { HANDRAILS_TEST_CAPTURE_PATH: capturePath },
        },
        context: {},
        authToken: "t",
        onLog: async () => {},
