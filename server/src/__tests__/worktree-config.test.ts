...
describe("worktree config repair", () => {
  it("repairs legacy repo-local worktree config and env files into an isolated instance", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-worktree-repair-"));
    const worktreeRoot = path.join(tempRoot, "PAP-884-ai-commits-component");
    const handrailsDir = path.join(worktreeRoot, ".handrails");
    const configPath = path.join(handrailsDir, "config.json");
    const envPath = path.join(handrailsDir, ".env");
    const sharedRoot = path.join(tempRoot, ".handrails", "instances", "default");
    const isolatedHome = path.join(tempRoot, ".handrails-worktrees");

    await fs.mkdir(handrailsDir, { recursive: true });
    await fs.writeFile(configPath, JSON.stringify(buildLegacyConfig(sharedRoot), null, 2) + "\n", "utf8");
    await fs.writeFile(
      envPath,
      [
        "# Handrails environment variables",
        "HANDRAILS_IN_WORKTREE=true",
        "HANDRAILS_WORKTREE_NAME=PAP-884-ai-commits-component",
        "HANDRAILS_AGENT_JWT_SECRET=shared-secret",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.env.HANDRAILS_IN_WORKTREE = "true";
    process.env.HANDRAILS_WORKTREE_NAME = "PAP-884-ai-commits-component";
    process.env.HANDRAILS_WORKTREES_DIR = isolatedHome;
    ...
    expect(repairedEnv).toContain(`HANDRAILS_HOME=${JSON.stringify(isolatedHome)}`);
    expect(repairedEnv).toContain('HANDRAILS_INSTANCE_ID="pap-884-ai-commits-component"');
    expect(repairedEnv).toContain(`HANDRAILS_CONFIG=${JSON.stringify(await fs.realpath(configPath))}`);
    expect(repairedEnv).toContain(`HANDRAILS_CONTEXT=${JSON.stringify(path.join(isolatedHome, "context.json"))}`);
    expect(repairedEnv).toContain('HANDRAILS_AGENT_JWT_SECRET="shared-secret"');
    expect(process.env.HANDRAILS_HOME).toBe(isolatedHome);
    expect(process.env.HANDRAILS_INSTANCE_ID).toBe("pap-884-ai-commits-component");
  });

  it("avoids sibling worktree ports when repairing legacy configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-worktree-repair-ports-"));
    ...
    await fs.writeFile(
      ...
      [
        "# Handrails environment variables",
        "HANDRAILS_IN_WORKTREE=true",
        "HANDRAILS_WORKTREE_NAME=PAP-880-thumbs-capture-for-evals-feature",
        "",
      ].join("\n"),
      "utf8",
    );
    ...
    process.chdir(worktreeRoot);
    process.env.HANDRAILS_IN_WORKTREE = "true";
    process.env.HANDRAILS_WORKTREE_NAME = "PAP-880-thumbs-capture-for-evals-feature";
    process.env.HANDRAILS_WORKTREES_DIR = isolatedHome;
  ...
  it("rebalances duplicate ports for already isolated worktree configs", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-worktree-rebalance-"));
    const isolatedHome = path.join(tempRoot, ".handrails-worktrees");
    const repoWorktreesRoot = path.join(tempRoot, "repo", ".handrails", "worktrees");
    ...
    const handrailsDir = path.join(currentWorktreeRoot, ".handrails");
    const configPath = path.join(handrailsDir, "config.json");
    const envPath = path.join(handrailsDir, ".env");
    const currentInstanceRoot = path.join(isolatedHome, "instances", "pap-884-ai-commits-component");
    const siblingConfigPath = path.join(siblingWorktreeRoot, ".handrails", "config.json");
  ...
  it("persists runtime-selected worktree ports back into config", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-worktree-ports-"));
    ...
    process.chdir(worktreeRoot);
    process.env.HANDRAILS_IN_WORKTREE = "true";
    process.env.HANDRAILS_WORKTREE_NAME = "PAP-878-create-a-mine-tab-in-inbox";
    process.env.HANDRAILS_HOME = isolatedHome;
    process.env.HANDRAILS_INSTANCE_ID = "pap-878-create-a-mine-tab-in-inbox";
    process.env.HANDRAILS_CONFIG = configPath;
