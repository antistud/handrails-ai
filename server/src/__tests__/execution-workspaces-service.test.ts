...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-execution-workspace-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.name", "Handrails Test"]);
  await runGit(repoRoot, ["config", "user.email", "test@handrails.local"]);
...
describeEmbeddedPostgres("executionWorkspaceService.getCloseReadiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("handrails-execution-workspaces-service-");
  ...
  it("warns about dirty and unmerged git worktrees and reports cleanup actions", async () => {
    ...
    const worktreePath = path.join(path.dirname(repoRoot), `handrails-worktree-${randomUUID()}`);
    tempDirs.add(worktreePath);

    await runGit(repoRoot, ["branch", "handrails-close-check"]);
    await runGit(repoRoot, ["worktree", "add", worktreePath, "handrails-close-check"]);
    ...
    await db.insert(executionWorkspaces).values({
      ...
      branchName: "handrails-close-check",
      baseRef: "main",
      metadata: {
