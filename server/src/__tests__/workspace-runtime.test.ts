...
import { resolveHandrailsConfigPath } from "../paths.ts";
import type { WorkspaceOperation } from "@handrailsai/shared";
...
async function createTempRepo() {
  const repoRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-worktree-repo-"));
  await runGit(repoRoot, ["init"]);
  await runGit(repoRoot, ["config", "user.email", "handrails@example.com"]);
  await runGit(repoRoot, ["config", "user.name", "Handrails Test"]);
...
afterEach(async () => {
  ...
  delete process.env.HANDRAILS_WORKTREES_DIR;
  delete process.env.DATABASE_URL;
  await resetRuntimeServicesForTests();
});

describe("sanitizeRuntimeServiceBaseEnv", () => {
  it("removes inherited Handrails and pnpm auth flags before spawning runtime services", () => {
    const sanitized = sanitizeRuntimeServiceBaseEnv({
      PATH: process.env.PATH,
      DATABASE_URL: "postgres://example.test/handrails",
      HANDRAILS_HOME: "/tmp/handrails-home",
      HANDRAILS_INSTANCE_ID: "runtime-instance",
      npm_config_tailscale_auth: "true",
      npm_config_authenticated_private: "true",
      HOST: "0.0.0.0",
    });

    expect(sanitized.HANDRAILS_HOME).toBeUndefined();
    expect(sanitized.HANDRAILS_INSTANCE_ID).toBeUndefined();
...
describe("realizeExecutionWorkspace", () => {
  it("creates and reuses a git worktree for an issue-scoped branch", async () => {
    ...
    expect(first.cwd).toContain(path.join(".handrails", "worktrees"));
  ...
  it("runs a configured provision command inside the derived worktree", async () => {
    ...
    await fs.writeFile(
      ...
      [
        "#!/usr/bin/env bash",
        "set -euo pipefail",
        "printf '%s\\n' \"$HANDRAILS_WORKSPACE_BRANCH\" > .handrails-provision-branch",
        "printf '%s\\n' \"$HANDRAILS_WORKSPACE_BASE_CWD\" > .handrails-provision-base",
        "printf '%s\\n' \"$HANDRAILS_WORKSPACE_CREATED\" > .handrails-provision-created",
      ].join("\n"),
      "utf8",
    );
    ...
    await expect(fs.readFile(path.join(workspace.cwd, ".handrails-provision-branch"), "utf8")).resolves.toBe(
      "PAP-448-run-provision-command\n",
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".handrails-provision-base"), "utf8")).resolves.toBe(
      `${repoRoot}\n`,
    );
    await expect(fs.readFile(path.join(workspace.cwd, ".handrails-provision-created"), "utf8")).resolves.toBe(
      "true\n",
    );
    ...
    await expect(fs.readFile(path.join(reused.cwd, ".handrails-provision-created"), "utf8")).resolves.toBe("false\n");
  });

  it("writes an isolated repo-local Handrails config and worktree branding when provisioning", async () => {
    const repoRoot = await createTempRepo();
    const previousCwd = process.cwd();
    const handrailsHome = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-worktree-home-"));
    const isolatedWorktreeHome = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-worktrees-"));
    const instanceId = "worktree-base";
    const sharedConfigDir = path.join(handrailsHome, "instances", instanceId);
    const sharedConfigPath = path.join(sharedConfigDir, "config.json");
    const sharedEnvPath = path.join(sharedConfigDir, ".env");

    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = instanceId;
    process.env.HANDRAILS_WORKTREES_DIR = isolatedWorktreeHome;
    ...
    await fs.writeFile(sharedEnvPath, 'DATABASE_URL="postgres://worktree:test@db.example.com:6543/handrails"\n', "utf8");
    ...
    try {
      ...
      const configPath = path.join(workspace.cwd, ".handrails", "config.json");
      const envPath = path.join(workspace.cwd, ".handrails", ".env");
      ...
      expect(envContents).toContain(`HANDRAILS_HOME=${JSON.stringify(isolatedWorktreeHome)}`);
      expect(envContents).toContain(`HANDRAILS_INSTANCE_ID=${JSON.stringify(expectedInstanceId)}`);
      expect(envContents).toContain(`HANDRAILS_CONFIG=${JSON.stringify(configPath)}`);
      expect(envContents).toContain("HANDRAILS_IN_WORKTREE=true");
      expect(envContents).toContain(
        `HANDRAILS_WORKTREE_NAME=${JSON.stringify("PAP-885-show-worktree-banner")}`,
      );

      process.chdir(workspace.cwd);
      expect(resolveHandrailsConfigPath()).toBe(configPath);
    } finally {
...
describe("ensureRuntimeServicesForRun", () => {
  it("reuses shared runtime services across runs and starts a new service after release", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-workspace-"));
  ...
  it("does not reuse project-scoped shared services across different workspace launch contexts", async () => {
    const primaryWorkspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-primary-"));
    const worktreeWorkspaceRoot = path.join(primaryWorkspaceRoot, ".handrails", "worktrees", "PAP-874-chat-speed-issues");
    ...
    const serviceCommand =
      "node -e \"require('node:http').createServer((req,res)=>res.end(process.env.HANDRAILS_HOME)).listen(Number(process.env.PORT), '127.0.0.1')\"";
    const config = {
      workspaceRuntime: {
        services: [
          {
            ...
            cwd: ".",
            env: {
              HANDRAILS_HOME: "{{workspace.cwd}}/.handrails/runtime-services",
            },
            port: { type: "auto" },
            readiness: {
              type: "http",
    ...
    expect(await primaryResponse.text()).toBe(path.join(primaryWorkspaceRoot, ".handrails", "runtime-services"));

    const executionResponse = await fetch(executionServices[0]!.url!);
    expect(await executionResponse.text()).toBe(path.join(worktreeWorkspaceRoot, ".handrails", "runtime-services"));
  });

  it("does not leak parent Handrails instance env into runtime service commands", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-env-"));
    const workspace = buildWorkspace(workspaceRoot);
    ...
    const serviceCommand = [
      ...
      JSON.stringify(
        [
          ...
          "handrailsConfig: process.env.HANDRAILS_CONFIG ?? null,",
          "handrailsHome: process.env.HANDRAILS_HOME ?? null,",
          "handrailsInstanceId: process.env.HANDRAILS_INSTANCE_ID ?? null,",
    ...
    process.env.HANDRAILS_CONFIG = "/tmp/base-handrails-config.json";
    process.env.HANDRAILS_HOME = "/tmp/base-handrails-home";
    process.env.HANDRAILS_INSTANCE_ID = "base-instance";
    process.env.DATABASE_URL = "postgres://shared-db.example.com/handrails";
    ...
    expect(captured.handrailsConfig).toBeNull();
    expect(captured.handrailsHome).toBeNull();
    expect(captured.handrailsInstanceId).toBeNull();
  ...
  it("stops execution workspace runtime services by executionWorkspaceId", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-stop-"));
    const workspace = buildWorkspace(workspaceRoot);
  ...
  it("does not stop services in sibling directories when matching by workspace cwd", async () => {
    const workspaceParent = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-sibling-"));
...
describeEmbeddedPostgres("workspace runtime startup reconciliation", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("handrails-workspace-runtime-");
  ...
  it("adopts a live auto-port shared service after runtime state is reset", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-reconcile-"));
    const handrailsHome = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-home-"));
    process.env.HANDRAILS_HOME = handrailsHome;
    process.env.HANDRAILS_INSTANCE_ID = `runtime-reconcile-${randomUUID()}`;
  ...
  it("persists controlled execution workspace stops as stopped", async () => {
    const workspaceRoot = await fs.mkdtemp(path.join(os.tmpdir(), "handrails-runtime-stop-persisted-"));
    const companyId = randomUUID();
    const agentId = randomUUID();
