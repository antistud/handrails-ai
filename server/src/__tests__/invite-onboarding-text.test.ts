...
describe("buildInviteOnboardingTextDocument", () => {
  it("renders a plain-text onboarding doc with expected endpoint references", () => {
    ...
    expect(text).toContain("Handrails OpenClaw Gateway Onboarding");
    ...
    expect(text).toContain("Suggested Handrails base URLs to try");
    expect(text).toContain("http://localhost:3100");
    expect(text).toContain("host.docker.internal");
    expect(text).toContain("handrailsApiUrl");
    ...
    expect(text).toContain("set the first reachable candidate as agentDefaultsPayload.handrailsApiUrl");
    expect(text).toContain("~/.openclaw/workspace/handrails-claimed-api-key.json");
    expect(text).toContain("HANDRAILS_API_KEY");
