...
import type { agents } from "@handrailsai/db";
import { sessionCodec as codexSessionCodec } from "@handrailsai/adapter-codex-local/server";
...
describe("formatRuntimeWorkspaceWarningLog", () => {
  it("emits informational workspace warnings on stdout", () => {
    expect(formatRuntimeWorkspaceWarningLog("Using fallback workspace")).toEqual({
      stream: "stdout",
      chunk: "[handrails] Using fallback workspace\n",
    });
  });
});
...
describe("parseSessionCompactionPolicy", () => {
  it("disables Handrails-managed rotation by default for codex and claude local", () => {
