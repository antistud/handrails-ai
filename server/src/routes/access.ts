...
import type { DeploymentExposure, DeploymentMode } from "@handrailsai/shared";
import {
  forbidden,
  conflict,
  notFound,
...
function readSkillMarkdown(skillName: string): string | null {
  ...
  if (
    normalized !== "handrails" &&
    normalized !== "handrails-create-agent" &&
    normalized !== "handrails-create-plugin" &&
...
/** Resolve the Handrails repo skills directory (built-in / managed skills). */
function resolveHandrailsSkillsDir(): string | null {
  const moduleDir = path.dirname(fileURLToPath(import.meta.url));
  const candidates = [
...
function listAvailableSkills(): AvailableSkill[] {
  ...
  const handrailsSkillsDir = resolveHandrailsSkillsDir();

  // Build set of Handrails-managed skill names
  const handrailsSkillNames = new Set<string>();
  if (handrailsSkillsDir) {
    try {
      for (const entry of fs.readdirSync(handrailsSkillsDir, { withFileTypes: true })) {
        if (entry.isDirectory()) handrailsSkillNames.add(entry.name);
      }
    } catch { /* skip */ }
  }
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      skills.push({
        name: entry.name,
        description,
        isHandrailsManaged: handrailsSkillNames.has(entry.name),
      });
    }
...
export function buildJoinDefaultsPayloadForAccept(input: {
  ...
  if (!nonEmptyTrimmedString(merged.handrailsApiUrl)) {
    const legacyHandrailsApiUrl = nonEmptyTrimmedString(input.handrailsApiUrl);
    if (legacyHandrailsApiUrl) merged.handrailsApiUrl = legacyHandrailsApiUrl;
  }
...
function summarizeOpenClawGatewayDefaultsForLog(defaultsPayload: unknown) {
  ...
  return {
    ...
    handrailsApiUrl: defaults
      ? nonEmptyTrimmedString(defaults.handrailsApiUrl)
      : null,
...
export function normalizeAgentDefaultsForJoin(input: {
  ...
  const rawHandrailsApiUrl =
    typeof defaults.handrailsApiUrl === "string"
      ? defaults.handrailsApiUrl.trim()
      : "";
  if (rawHandrailsApiUrl) {
    try {
      const parsedHandrailsApiUrl = new URL(rawHandrailsApiUrl);
      if (
        parsedHandrailsApiUrl.protocol !== "http:" &&
        parsedHandrailsApiUrl.protocol !== "https:"
      ) {
        diagnostics.push({
          code: "openclaw_gateway_handrails_api_url_protocol",
          level: "warn",
          message: `handrailsApiUrl must use http:// or https:// (got ${parsedHandrailsApiUrl.protocol}).`
        });
      } else {
        normalized.handrailsApiUrl = parsedHandrailsApiUrl.toString();
        diagnostics.push({
          code: "openclaw_gateway_handrails_api_url_configured",
          level: "info",
          message: `handrailsApiUrl set to ${parsedHandrailsApiUrl.toString()}`
        });
      }
    } catch {
      diagnostics.push({
        code: "openclaw_gateway_handrails_api_url_invalid",
        level: "warn",
        message: `Invalid handrailsApiUrl: ${rawHandrailsApiUrl}`
      });
    }
  }
...
function buildOnboardingDiscoveryDiagnostics(input: {
  ...
  if (apiHost && isLoopbackHost(apiHost)) {
    diagnostics.push({
      ...
      level: "warn",
      message:
        "Onboarding URL resolves to loopback hostname. Remote OpenClaw agents cannot reach localhost on your Handrails host.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      level: "warn",
      message: "Handrails is bound to loopback in authenticated/private mode.",
  ...
  if (
    ...
    diagnostics.push({
      ...
      hint: `Run pnpm handrailsai allowed-hostname ${apiHost}`
    });
  }
...
function buildInviteOnboardingManifest(
  ...
  const baseUrl = requestBaseUrl(req);
  const skillPath = "/api/skills/handrails";
  ...
  return {
    ...
    onboarding: {
      instructions:
        "Join as an OpenClaw Gateway agent, save your one-time claim secret, wait for board approval, then claim your API key. Save the claim response token to ~/.openclaw/workspace/handrails-claimed-api-key.json and load HANDRAILS_API_KEY from that file before starting heartbeat loops. You MUST submit adapterType='openclaw_gateway', set agentDefaultsPayload.url to your ws:// or wss:// OpenClaw gateway endpoint, and include agentDefaultsPayload.headers.x-openclaw-token (or legacy x-openclaw-auth).",
      ...
      requiredFields: {
        ...
        agentDefaultsPayload:
          "Adapter config for OpenClaw gateway. MUST include url (ws:// or wss://) and headers.x-openclaw-token (or legacy x-openclaw-auth). Optional fields: handrailsApiUrl, waitTimeoutMs, sessionKeyStrategy, sessionKey, role, scopes, disableDeviceAuth, devicePrivateKeyPem."
      },
      ...
      connectivity: {
        ...
        guidance:
          ...
          opts.deploymentExposure === "private"
            ? "If OpenClaw runs on another machine, ensure the Handrails hostname is reachable and allowed via `pnpm handrailsai allowed-hostname <host>`."
            : "Ensure OpenClaw can reach this Handrails API base URL for invite, claim, and skill bootstrap calls."
      },
      textInstructions: {
      ...
      skill: {
        ...
        path: skillPath,
        url: skillUrl,
        installPath: "~/.openclaw/skills/handrails/SKILL.md"
      }
    }
  };
}
...
export function buildInviteOnboardingTextDocument(
  ...
  appendBlock(`
    # Handrails OpenClaw Gateway Onboarding
  ...
  appendBlock(`
    ...
    BODY="$(node -e '
      ...
      const body = {
        ...
        agentDefaultsPayload: {
          ...
          handrailsApiUrl: "http://host.docker.internal:3100",
    ...
    - Keep device auth enabled (recommended). If devicePrivateKeyPem is omitted, Handrails generates and persists one during join so pairing approvals are stable.
    ...
    Body (JSON):
    {
      ...
      "agentDefaultsPayload": {
        ...
        "handrailsApiUrl": "https://handrails-hostname-your-agent-can-reach:3100",
    ...
    The board approves the join request in Handrails before key claim is allowed.
    ...
    - ~/.openclaw/workspace/handrails-claimed-api-key.json
    chmod 600 ~/.openclaw/workspace/handrails-claimed-api-key.json

    And set the HANDRAILS_API_KEY and HANDRAILS_API_URL in your environment variables as specified here:
    ...
    Then set HANDRAILS_API_KEY and HANDRAILS_API_URL from the saved token field for every heartbeat run.
    ...
    ## Step 4: Install Handrails skill in OpenClaw
    GET ${onboarding.skill.url}
    Install path: ${onboarding.skill.installPath}

    Be sure to prepend your HANDRAILS_API_URL to the top of your skill and note the path to your HANDRAILS_API_URL
    ...
    ${
      ...
      "Ensure Handrails is reachable from your OpenClaw runtime."
    }
  `);
  ...
  if (connectionCandidates.length > 0) {
    lines.push("## Suggested Handrails base URLs to try");
    ...
    appendBlock(`
      ...
      - set the first reachable candidate as agentDefaultsPayload.handrailsApiUrl when submitting your join request

      If none are reachable: ask your human operator for a reachable hostname/address and help them update network configuration.
      For authenticated/private mode, they may need:
      - pnpm handrailsai allowed-hostname <host>
      - then restart Handrails and retry onboarding.
    `);
  }
...
async function resolveActorEmail(db: Db, req: Request): Promise<string | null> {
  if (isLocalImplicit(req)) return "local@handrails.local";
...
export function accessRoutes(
  ...
  router.get("/skills/index", (_req, res) => {
    res.json({
      skills: [
        { name: "handrails", path: "/api/skills/handrails" },
        {
          name: "para-memory-files",
          path: "/api/skills/para-memory-files"
        },
        {
          name: "handrails-create-agent",
          path: "/api/skills/handrails-create-agent"
        }
      ]
    });
  });
  ...
  router.post(
    ...
    async (req, res) => {
      ...
      const gatewayDefaultsPayload =
        requestType === "agent"
          ? buildJoinDefaultsPayloadForAccept({
              ...
              handrailsApiUrl: req.body.handrailsApiUrl ?? null,
      ...
      if (requestType === "agent" && adapterType === "openclaw_gateway") {
        ...
        if (
          expectedDefaults.handrailsApiUrl &&
          !persistedDefaults.handrailsApiUrl
        ) {
          missingPersistedFields.push("handrailsApiUrl");
        }
