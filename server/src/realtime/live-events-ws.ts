...
import { agentApiKeys, companyMemberships, instanceUserRoles } from "@handrailsai/db";
import type { DeploymentMode } from "@handrailsai/shared";
...
interface IncomingMessageWithContext extends IncomingMessage {
  handrailsUpgradeContext?: UpgradeContext;
}
...
export function setupLiveEventsWebSocketServer(
  ...
  wss.on("connection", (socket: WsSocket, req: IncomingMessage) => {
    const context = (req as IncomingMessageWithContext).handrailsUpgradeContext;
    if (!context) {
  ...
  server.on("upgrade", (req, socket, head) => {
    ...
    void authorizeUpgrade(db, req, companyId, url, {
      ...
      .then((context) => {
        ...
        reqWithContext.handrailsUpgradeContext = context;
