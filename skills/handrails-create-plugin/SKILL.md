---
name: handrails-create-plugin
description: >
  Create new Handrails plugins with the current alpha SDK/runtime. Use when
...
# Create a Handrails Plugin

Use this skill when the task is to create, scaffold, or document a Handrails plugin.
...
node packages/plugins/create-handrails-plugin/dist/index.js <npm-package-name> --output <target-dir>
```

For a plugin that lives outside the Handrails repo, pass `--sdk-path` and let the scaffold snapshot the local SDK/shared packages into `.handrails-sdk/`:

```bash
pnpm --filter @handrailsai/create-handrails-plugin build
node packages/plugins/create-handrails-plugin/dist/index.js @acme/plugin-name \
...
- is installed into Handrails from an absolute local path during development
