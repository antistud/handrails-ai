/**
 * esbuild configuration for building the handrailsai CLI for npm.
 *
 * Bundles all workspace packages (@handrailsai/*) into a single file.
...
// to npm and resolved at runtime (e.g. @handrailsai/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@handrailsai/")) {
      externals.add(name);
    }
  }
