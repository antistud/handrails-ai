...
const JWT_SECRET_ENV_KEY = "HANDRAILS_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    ...
    "# Generated by Handrails CLI commands",
...
export function resolveHandrailsEnvFile(configPath?: string): string {
...
export function loadHandrailsEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeHandrailsEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readHandrailsEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeHandrailsEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeHandrailsEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readHandrailsEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeHandrailsEnvEntries(next, filePath);
  return next;
}
