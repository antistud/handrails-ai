...
import { ApiConnectionError, ApiRequestError, HandrailsApiClient } from "../client/http.js";

describe("HandrailsApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new HandrailsApiClient({
    ...
    expect(headers["x-handrails-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new HandrailsApiClient({ apiBase: "http://localhost:3100" });
