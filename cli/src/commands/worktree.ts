...
import { ensureAgentJwtSecret, loadHandrailsEnvFile, mergeHandrailsEnvEntries, readHandrailsEnvEntries, resolveHandrailsEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { HandrailsConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printHandrailsCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.HANDRAILS_CONFIG;
...
const WORKTREE_NAME_PREFIX = "handrails-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.HANDRAILS_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.HANDRAILS_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromHandrailsConfig(config: HandrailsConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromHandrailsConfig(config);
}
...
function resolveRepoManagedWorktreesRoot(cwd: string): string | null {
  ...
  const marker = `${path.sep}.handrails${path.sep}worktrees${path.sep}`;
  const index = normalized.indexOf(marker);
  if (index === -1) return null;
  const repoRoot = normalized.slice(0, index);
  return path.resolve(repoRoot, ".handrails", "worktrees");
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".handrails", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.handrails"));
  const sourceInstanceId = sanitizeWorktreeInstanceId(opts.fromInstance ?? "default");
  return path.resolve(sourceHome, "instances", sourceInstanceId, "config.json");
}

function resolveSourceConnectionString(config: HandrailsConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://handrails:handrails@127.0.0.1:${port}/handrails`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: HandrailsConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.HANDRAILS_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HANDRAILS_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.HANDRAILS_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HANDRAILS_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: HandrailsConfig;
  targetConfig: HandrailsConfig;
  targetPaths: WorktreeLocalPaths;
  instanceId: string;
  seedMode: WorktreeSeedMode;
}): Promise<SeedWorktreeDatabaseResult> {
  const seedPlan = resolveWorktreeSeedPlan(input.seedMode);
  const sourceEnvFile = resolveHandrailsEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readHandrailsEnvEntries(sourceEnvFile);
  ...
  try {
    ...
    const adminConnectionString = `postgres://handrails:handrails@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "handrails");
    const targetConnectionString = `postgres://handrails:handrails@127.0.0.1:${targetHandle.port}/handrails`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  const sourceEnvEntries = readHandrailsEnvEntries(resolveHandrailsEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.HANDRAILS_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.HANDRAILS_AGENT_JWT_SECRET);
  mergeHandrailsEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { HANDRAILS_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadHandrailsEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run Handrails inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printHandrailsCliBanner();
  p.intro(pc.bgCyan(pc.black(" handrailsai worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printHandrailsCliBanner();
  p.intro(pc.bgCyan(pc.black(" handrailsai worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasHandrailsConfig: existsSync(path.resolve(worktreePath, ".handrails", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printHandrailsCliBanner();
  p.intro(pc.bgCyan(pc.black(" handrailsai worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveHandrailsEnvFile(configPath);
  const envEntries = readHandrailsEnvEntries(envPath);
  const out = {
    HANDRAILS_CONFIG: configPath,
    ...(envEntries.HANDRAILS_HOME ? { HANDRAILS_HOME: envEntries.HANDRAILS_HOME } : {}),
    ...(envEntries.HANDRAILS_INSTANCE_ID ? { HANDRAILS_INSTANCE_ID: envEntries.HANDRAILS_INSTANCE_ID } : {}),
    ...(envEntries.HANDRAILS_CONTEXT ? { HANDRAILS_CONTEXT: envEntries.HANDRAILS_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasHandrailsConfig)
      .map((choice) => path.resolve(choice.worktree, ".handrails", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readHandrailsEnvEntries(resolveHandrailsEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start Handrails once) before using worktree merge history.`,
      );
    }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasHandrailsConfig ? "handrails" : "no-handrails-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".handrails", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".handrails", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .handrails/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasHandrailsConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a Handrails worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasHandrailsConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No Handrails worktrees were found. Run `handrailsai worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target Handrails configs are the same. Choose different --from/--to worktrees.");
  }
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local Handrails instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated Handrails instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with handrails- if needed (created at ~/handrails-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: HANDRAILS_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: HANDRAILS_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source HANDRAILS_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local Handrails instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like Handrails worktrees")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with handrails- if needed")
