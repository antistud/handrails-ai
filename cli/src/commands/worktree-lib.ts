...
export const DEFAULT_WORKTREE_HOME = "~/.handrails-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".handrails");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: HandrailsConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "handrails",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    HANDRAILS_HOME: paths.homeDir,
    HANDRAILS_INSTANCE_ID: paths.instanceId,
    HANDRAILS_CONFIG: paths.configPath,
    HANDRAILS_CONTEXT: paths.contextPath,
    HANDRAILS_IN_WORKTREE: "true",
    ...(branding?.name ? { HANDRAILS_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { HANDRAILS_WORKTREE_COLOR: branding.color } : {}),
  };
}
