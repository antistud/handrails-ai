...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@handrailsai/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://handrails:handrails@127.0.0.1:${port}/handrails`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" handrails db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveHandrailsInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "handrails";
