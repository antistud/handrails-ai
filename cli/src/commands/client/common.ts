...
import { ApiRequestError, HandrailsApiClient } from "../../client/http.js";
...
export function addCommonClientOptions(command: Command, opts?: { includeCompany?: boolean }): Command {
  command
    .option("-c, --config <path>", "Path to Handrails config file")
    .option("-d, --data-dir <path>", "Handrails data directory root (isolates state from ~/.handrails)")
    .option("--context <path>", "Path to CLI context file")
    .option("--profile <name>", "CLI context profile name")
    .option("--api-base <url>", "Base URL for the Handrails API")
...
export function resolveCommandContext(
  ...
  const apiBase =
    ...
    process.env.HANDRAILS_API_URL?.trim() ||
    profile.apiBase ||
    inferApiBaseFromConfig(options.config);

  const explicitApiKey =
    options.apiKey?.trim() ||
    process.env.HANDRAILS_API_KEY?.trim() ||
  ...
  const companyId =
    ...
    process.env.HANDRAILS_COMPANY_ID?.trim() ||
    profile.companyId;

  if (opts?.requireCompany && !companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set HANDRAILS_COMPANY_ID, or set context profile companyId via `handrailsai context set`.",
    );
  }

  const api = new HandrailsApiClient({
    apiBase,
    apiKey,
...
function inferApiBaseFromConfig(configPath?: string): string {
  const envHost = process.env.HANDRAILS_SERVER_HOST?.trim() || "localhost";
  let port = Number(process.env.HANDRAILS_SERVER_PORT || "");
