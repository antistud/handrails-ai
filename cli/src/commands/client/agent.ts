...
import type { Agent } from "@handrailsai/shared";
import {
  removeMaintainerOnlySkillSymlinks,
  resolveHandrailsSkillsDir,
...
function buildAgentEnvExports(input: {
  ...
  return [
    `export HANDRAILS_API_URL='${escaped(input.apiBase)}'`,
    `export HANDRAILS_COMPANY_ID='${escaped(input.companyId)}'`,
    `export HANDRAILS_AGENT_ID='${escaped(input.agentId)}'`,
    `export HANDRAILS_API_KEY='${escaped(input.apiKey)}'`,
  ].join("\n");
}
...
export function registerAgentCommands(program: Command): void {
  ...
  addCommonClientOptions(
    agent
      .command("local-cli")
      .description(
        "Create an agent API key, install local Handrails skills for Codex/Claude, and print shell exports",
      )
      ...
      .option(
        "--no-install-skills",
        "Skip installing Handrails skills into ~/.codex/skills and ~/.claude/skills",
      )
      .action(async (agentRef: string, opts: AgentLocalCliOptions) => {
        try {
          ...
          if (opts.installSkills !== false) {
            const skillsDir = await resolveHandrailsSkillsDir(__moduleDir, [path.resolve(process.cwd(), "skills")]);
            if (!skillsDir) {
              throw new Error(
                "Could not locate local Handrails skills directory. Expected ./skills in the repo checkout.",
              );
            }
