...
type OnboardDefaults = Pick<HandrailsConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const ONBOARD_ENV_KEYS = [
  "HANDRAILS_PUBLIC_URL",
  "DATABASE_URL",
  "HANDRAILS_DB_BACKUP_ENABLED",
  "HANDRAILS_DB_BACKUP_INTERVAL_MINUTES",
  "HANDRAILS_DB_BACKUP_RETENTION_DAYS",
  "HANDRAILS_DB_BACKUP_DIR",
  "HANDRAILS_DEPLOYMENT_MODE",
  "HANDRAILS_DEPLOYMENT_EXPOSURE",
  "HOST",
  "PORT",
  "SERVE_UI",
  "HANDRAILS_ALLOWED_HOSTNAMES",
  "HANDRAILS_AUTH_BASE_URL_MODE",
  "HANDRAILS_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "HANDRAILS_STORAGE_PROVIDER",
  "HANDRAILS_STORAGE_LOCAL_DIR",
  "HANDRAILS_STORAGE_S3_BUCKET",
  "HANDRAILS_STORAGE_S3_REGION",
  "HANDRAILS_STORAGE_S3_ENDPOINT",
  "HANDRAILS_STORAGE_S3_PREFIX",
  "HANDRAILS_STORAGE_S3_FORCE_PATH_STYLE",
  "HANDRAILS_SECRETS_PROVIDER",
  "HANDRAILS_SECRETS_STRICT_MODE",
  "HANDRAILS_SECRETS_MASTER_KEY_FILE",
] as const;
...
function quickstartDefaultsFromEnv(): {
  ...
  const instanceId = resolveHandrailsInstanceId();
  const defaultStorage = defaultStorageConfig();
  const defaultSecrets = defaultSecretsConfig();
  const databaseUrl = process.env.DATABASE_URL?.trim() || undefined;
  const publicUrl =
    process.env.HANDRAILS_PUBLIC_URL?.trim() ||
    process.env.HANDRAILS_AUTH_PUBLIC_BASE_URL?.trim() ||
    process.env.BETTER_AUTH_URL?.trim() ||
    process.env.BETTER_AUTH_BASE_URL?.trim() ||
    undefined;
  const deploymentMode =
    parseEnumFromEnv<DeploymentMode>(process.env.HANDRAILS_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted";
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.HANDRAILS_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  ...
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.HANDRAILS_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.HANDRAILS_ALLOWED_HOSTNAMES
    ? process.env.HANDRAILS_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.HANDRAILS_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.HANDRAILS_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.HANDRAILS_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.HANDRAILS_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.HANDRAILS_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.HANDRAILS_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.HANDRAILS_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.HANDRAILS_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.HANDRAILS_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.HANDRAILS_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.HANDRAILS_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.HANDRAILS_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.HANDRAILS_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.HANDRAILS_SECRETS_MASTER_KEY_FILE) ??
          defaultSecrets.localEncrypted.keyFilePath,
      },
    },
  };
  const ignoredEnvKeys: Array<{ key: string; reason: string }> = [];
  if (deploymentMode === "local_trusted" && process.env.HANDRAILS_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "HANDRAILS_DEPLOYMENT_EXPOSURE",
...
function canCreateBootstrapInviteImmediately(config: Pick<HandrailsConfig, "database" | "server">): boolean {
  return config.server.deploymentMode === "authenticated" && config.database.mode !== "embedded-postgres";
}

export async function onboard(opts: OnboardOptions): Promise<void> {
  printHandrailsCliBanner();
  p.intro(pc.bgCyan(pc.black(" handrailsai onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveHandrailsInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: HandrailsConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing Handrails install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("handrailsai configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("HANDRAILS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.HANDRAILS_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("HANDRAILS_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("HANDRAILS_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: HANDRAILS_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("handrailsai run")}`,
        `Reconfigure later: ${pc.cyan("handrailsai configure")}`,
        `Diagnose setup: ${pc.cyan("handrailsai doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.HANDRAILS_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing Handrails setup is ready.");
    return;
  }
  ...
  let llm: HandrailsConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@handrailsai/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `handrailsai doctor`"));
      }
    }
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("handrailsai run")}`,
        `Then: ${pc.cyan("handrailsai auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
