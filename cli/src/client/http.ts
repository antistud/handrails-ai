...
export class HandrailsApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-handrails-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the Handrails API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the Handrails server is not running, the configured URL is wrong, or the request is being blocked before it reaches Handrails.",
    "",
    "Try:",
    "- Start Handrails with `pnpm dev` or `pnpm handrailsai run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If Handrails is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`HANDRAILS_API_URL\`.`,
  );
  return lines.join("\n");
}
