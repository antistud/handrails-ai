...
- Running a Python script that calls the Handrails API
...
1. Handrails spawns the configured command as a child process
2. Standard Handrails environment variables are injected (`HANDRAILS_AGENT_ID`, `HANDRAILS_API_KEY`, etc.)
...
The script can use the injected environment variables to authenticate with the Handrails API and perform work.
