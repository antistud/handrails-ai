...
Build a custom adapter to connect Handrails to any agent runtime.
...
2. Build environment with `buildHandrailsEnv(agent)` plus context vars
...
`format-event.ts` — pretty-prints stdout for `handrailsai run --watch` using `picocolors`.
...
Make Handrails skills discoverable to your agent runtime without writing to the agent's working directory:
