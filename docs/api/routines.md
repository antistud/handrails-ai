...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-Handrails-Signature` + `X-Handrails-Timestamp` header pair matching the trigger's signing mode.
