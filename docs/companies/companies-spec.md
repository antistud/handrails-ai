...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Handrails.
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.handrails.yaml`
...
- Handrails must not require extra top-level fields for skill validity
- Handrails-specific extensions must live under `metadata.handrails` or `metadata.sources`
...
For Handrails, the preferred fidelity extension is:
...
- issue/task Handrails-only metadata
...
- Handrails tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Handrails shape:
...
Additional rules for Handrails exporters:
...
- omit empty and default-valued Handrails fields when possible
...
- Handrails exporters should emit `.handrails.yaml` as a sidecar by default
...
## 19. Handrails Mapping

Handrails can map this spec to its runtime model like this:
...
- Handrails extension:
  - `.handrails.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Handrails-specific fidelity

Inline Handrails-only metadata that must live inside a shared markdown file should use:

- `metadata.handrails`

That keeps the base format broader than Handrails.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Handrails.

## 20. Cutover

Handrails should cut over to this markdown-first package model as the primary portability format.

`handrails.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Handrails, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
