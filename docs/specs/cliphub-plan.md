# ClipHub: Marketplace for Handrails Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Handrails configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Handrails-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Handrails company (agents, projects, governance) |
...
A complete Handrails company configuration:
...
Single-agent configurations designed to plug into a Handrails org:
...
Portable skill files that any Handrails agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Handrails's skill loading system
...
interface Listing {
  ...
  handrailsVersionMin: string;     // Minimum Handrails version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target Handrails company
...
| `POST` | `/api/listings/:id/install` | Install to Handrails company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target Handrails company (or create new)
  → ClipHub API calls Handrails API to:
  ...
  → Redirect to Handrails dashboard with new team running
```
...
### 5.3 Creator: Export from Handrails → Publish

```
Running Handrails company → "Export as Blueprint" (CLI or UI)
  → Handrails exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Handrails brand blue/purple)
...
| Install flow | One-click deploy to Handrails company |
...
{
  "targetCompanyId": "uuid",         // Existing Handrails company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `handrails-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Handrails
- **Backend**: Node.js API (or extend Handrails server)
- **Database**: Postgres (can share Handrails's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Handrails auth (or OAuth2)

### 9.2 Integration with Handrails

ClipHub can be:
- **Option A**: A separate app that calls Handrails's API to install blueprints
- **Option B**: A built-in section of the Handrails UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Handrails UI and API.
...
- [ ] Install flow: blueprint → Handrails company
...
- [ ] "Export from Handrails" CLI command
