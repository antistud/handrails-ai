...
How to get OpenClaw running in a Docker container for local development and testing the Handrails OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Handrails includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-handrails-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Handrails host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-handrails-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `HANDRAILS_HOST_PORT` (default `3100`)
- `HANDRAILS_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Handrails deployment is `authenticated`, provide auth context:

```bash
HANDRAILS_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
HANDRAILS_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Handrails server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Handrails URL (typically `http://host.docker.internal:3100`).
- If Handrails rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm handrailsai allowed-hostname host.docker.internal
```

Then restart Handrails and rerun the smoke script.
