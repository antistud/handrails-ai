...
Handrails companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .handrails.yaml     # Adapter config, env inputs, routines
```
...
- **.handrails.yaml** holds Handrails-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
handrailsai company export <company-id> --out ./my-export
```
...
handrailsai company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
handrailsai company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
handrailsai company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.handrails.yaml`
...
```sh
# From a local folder
handrailsai company import ./my-export

# From a GitHub URL
handrailsai company import https://github.com/org/repo

# From a GitHub subfolder
handrailsai company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
handrailsai company import org/repo
handrailsai company import org/repo/companies/acme
```
...
If `--target` is not specified, Handrails infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
handrailsai company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
handrailsai company import org/company-templates/engineering-team \
  --target new \
...
```sh
handrailsai company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
handrailsai company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
handrailsai company import ./package \
  --target new \
  --yes \
  --json
```
...
Handrails supports several GitHub URL formats:
