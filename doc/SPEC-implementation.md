# Handrails V1 Implementation Spec
...
Handrails V1 must provide a full control-plane loop for autonomous agents:
...
- Primary: PostgreSQL
- Local default: embedded PostgreSQL at `~/.handrails/instances/default/db`
- Optional local prod-like: Docker Postgres
- Optional hosted: Supabase/Postgres-compatible
- File/object storage:
  - local default: `~/.handrails/instances/default/data/storage` (`local_disk`)
...
- `.handrails.yaml` sidecar for Handrails-specific fidelity
...
- export emits a clean vendor-neutral markdown package plus `.handrails.yaml`
- projects and starter tasks are opt-in export content rather than default package content
- recurring `TASK.md` entries use `recurring: true` in the base package and Handrails routine fidelity in `.handrails.yaml`
- Handrails imports recurring task packages as routines instead of downgrading them to one-time issues
- export strips environment-specific paths (`cwd`, local instruction file paths, inline prompt duplication) while preserving portable project repo/workspace metadata such as `repoUrl`, refs, and workspace-policy references keyed in `.handrails.yaml`
