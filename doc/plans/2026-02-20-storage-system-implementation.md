...
Add a single storage subsystem for Handrails that supports:
...
- API serves attachment bytes through authenticated Handrails endpoints.
...
- Default local path is under instance root: `~/.handrails/instances/<instanceId>/data/storage`.
...
- `handrailsai onboard` writes a valid `storage` config block by default.
- `handrailsai configure --section storage` can switch between local and s3 modes.
...
- `handrailsai doctor` reports actionable storage status.
