...
This document defines the rollout plan for adapter-wide skill support in Handrails.
...
- Handrails stores desired skill state consistently even when the adapter cannot fully reconcile it
- unsupported adapters degrade clearly and safely

## 2. Current Adapter Matrix

Handrails currently has these adapters:
...
1. Desired skills live in Handrails for every adapter.
...
These adapters have a stable local skills directory that Handrails can read and manage.
...
These adapters do not have a meaningful Handrails-owned persistent install state.
...
- desired skills stored in Handrails
...
- runtime already injects Handrails skills into `~/.cursor/skills`
...
- runtime already injects Handrails skills into `~/.gemini/skills`
...
- runtime already injects Handrails skills into `~/.pi/agent/skills`
...
- Handrails can sync desired skills into Pi’s persistent home
...
- OpenCode currently injects Handrails skills into `~/.claude/skills`

This is product-risky because:

- it shares state with Claude
- Handrails may accidentally imply the skills belong only to OpenCode when the home is shared
...
- only remove stale managed Handrails skills that are clearly marked as Handrails-managed
...
- Handrails stores desired skills only
...
- avoid destructive wording unless Handrails can prove a skill is Handrails-managed
...
That gets Handrails from “skills work for Codex and Claude” to “skills work for the whole local-adapter family,” which is the meaningful V1 milestone.
