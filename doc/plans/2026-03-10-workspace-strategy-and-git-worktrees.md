...
`PAP-447` asks how Handrails should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Handrails instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Handrails:

- some operators use Handrails to manage Handrails and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Handrails
- Claude and Codex expose different built-in affordances, so Handrails should not overfit to one tool

## Core Product Decision

Handrails should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Handrails
...
- Handrails should not assume that every adapter can see or use a host filesystem path directly
...
If Handrails makes worktrees agent-first, it will blur:
...
- Handrails core resolves and records execution workspace state
...
That gives Handrails room to support:
...
- Handrails handles workspace runtime services under the hood using project/workspace policy
...
Once Handrails is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Handrails should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Handrails cannot infer
...
- when issue moves to done, should Handrails auto-commit?
- should it auto-push?
...
Handrails already has the right foundation for a project-first model.
...
For server-spawned local adapters, Handrails already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Handrails should mirror directly

Implication:

- for `codex_local`, Handrails should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Handrails should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Handrails should separate:
...
Handrails should be able to express intentions such as:
...
These are not specific to Handrails. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".handrails/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Handrails-specific `devServer` flag.
...
Handrails should distinguish between:
...
Handrails should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Handrails should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Handrails
...
For runtime services specifically, Handrails should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Handrails-only dev-server flags
...
3. Inject resolved execution workspace details into `context.handrailsWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Handrails instance can be reused through env/config injection when appropriate
...
- Handrails can decide whether to reuse or start a fresh service deterministically
...
3. Test against Handrails-on-Handrails workflows first.
...
7. Workspace runtime services are modeled generically, not as Handrails-specific dev-server toggles.
