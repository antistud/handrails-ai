...
Agents need API keys to authenticate with Handrails. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Handrails
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Handrails generates a token and passes it directly to the agent
...
`HANDRAILS_API_URL`, `HANDRAILS_AGENT_ID`, `HANDRAILS_COMPANY_ID`. We need to
add a `HANDRAILS_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Handrails CLI
...
```
handrails auth login
# Opens browser -> user confirms -> token stored at ~/.handrails/credentials
```
...
managed by the Handrails server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Handrails.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Handrails SKILL.md (or a link to it)
   - What information Handrails needs from the agent (e.g. webhook URL, adapter
...
5. Handrails stores the pending registration.
...
7. On approval, Handrails provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Handrails issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.handrails.ing/skills/handrails/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How Handrails should send you heartbeats",
...
1. Generate a Handrails invite link for the company.
...
4. A Handrails company member approves the new agent.
5. Handrails begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `HANDRAILS_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `handrailsai auth login` for developer-managed remote agents.                                      |
