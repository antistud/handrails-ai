...
Handrails already treats budgets as a core control-plane responsibility:
...
This plan defines the concrete budgeting model Handrails should implement next.

## Product Goals

Handrails should let operators:
...
For the next implementation pass, Handrails should enforce these defaults:
...
Handrails should separate subscription-included usage from billed spend:
...
Handrails should have two threshold classes:
...
Handrails must know that a project is budget-paused and enforce it.
...
Handrails must also block execution before new work starts.
