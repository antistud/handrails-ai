...
This document defines the V1 design for a Handrails adapter that integrates with
...
4. synthesized stdout events for Handrails UI/CLI
...
3. **Agent callback auth to Handrails** uses a bootstrap exchange flow (no long-lived Handrails key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Handrails endpoints, not full SKILL.md prompt injection.
...
- `handrailsPublicUrl` (optional override; else `HANDRAILS_PUBLIC_URL` env)
...
## Handrails Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `HANDRAILS_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.handrailsPublicUrl`
2. `process.env.HANDRAILS_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Handrails credentials in prompt text.

Flow:

1. Before launch/follow-up, Handrails mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Handrails validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Handrails API calls.
...
1. Prompt includes a compact instruction to fetch skills from Handrails.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/handrails`
   - `GET /api/skills/handrails-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <handrailsPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Handrails run context.
...
5. `handrailsPublicUrl` present and reachable shape-valid
...
| Skill delivery | local skill dir injection | authenticated fetch from Handrails skill endpoints |
| Handrails auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Handrails context.
3. Add richer PR metadata surfacing in Handrails UI.
