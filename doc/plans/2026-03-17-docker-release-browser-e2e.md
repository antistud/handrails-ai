...
Today release smoke testing for published Handrails packages is manual and shell-driven:
...
- Docker
- `npx handrailsai@canary`
- `npx handrailsai@latest`
...
- runs `handrailsai@${HANDRAILSAI_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@handrails.local`
- password: `handrails-smoke-password`
...
- the suite passes locally against both:
  - `HANDRAILSAI_VERSION=canary`
  - `HANDRAILSAI_VERSION=latest`
...
1. A published `handrailsai@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `handrailsai@latest` Docker install can be smoke-tested by Playwright in CI.
