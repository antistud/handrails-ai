...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/handrails-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `HANDRAILS_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `HANDRAILS_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Handrails may create host-local worktrees, processes, and paths
- `cloud`
  - Handrails should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `handrails | github | vercel | s3 | custom`
...
- use `HANDRAILS_EXECUTION_TOPOLOGY` as a defaulting guardrail
