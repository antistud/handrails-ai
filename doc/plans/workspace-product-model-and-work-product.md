...
Handrails needs to support two very different but equally valid ways of working:
...
Today, Handrails already has the beginnings of this model:
...
- making Handrails feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Handrails users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Handrails deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Handrails into a full code review product
...
This object must be recorded explicitly so that Handrails can:
...
Handrails should treat PRs as a type of work product linked back to:
...
Handrails already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Handrails must be a first-class deployment mode

Handrails cannot assume that it is running on the same machine as the code.

In cloud deployments, Handrails may:
...
- `Runtime service`: a process or service Handrails owns or tracks for a workspace
...
It also lets cloud-hosted Handrails deployments point at codebases and remotes without pretending the Handrails host has direct filesystem access.
...
Without an explicit `execution workspace` record, Handrails has nowhere stable to attach:
...
Handrails needs a single place to show:
...
- Handrails-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Handrails owning the execution host
...
## 1. Handrails host and execution host must be decoupled
...
without the Handrails host itself running local git or local preview processes.
...
Handrails should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Handrails.
...
Handrails should keep workspace automation centered on a small set of hooks:
...
Handrails should provide the equivalent visibility in the project `Code` surface:
...
Handrails should treat execution workspaces as the natural home for:
...
Handrails should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Handrails should continue treating cleanup policy as part of the core workspace model:
...
Handrails should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Handrails host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Handrails should borrow Worktrunk's good execution patterns while keeping the broader Handrails model:
...
This is how Handrails supports:
...
That keeps Handrails focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Handrails deployments remain first-class

That is a better fit for Handrails than either extreme:
