...
This document defines the next-stage plan for Handrails company import/export.

The core shift is:

- move from a Handrails-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Handrails.
...
Handrails already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Handrails
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `handrails.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.handrails.yaml` sidecar for high-fidelity Handrails-specific details
...
11. adapter-aware skill sync surfaces so Handrails can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Handrails at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Handrails sidecar.
...
Handrails must not redefine `SKILL.md`.
...
- Handrails-specific fidelity lives in `.handrails.yaml`
- Handrails may resolve and install `SKILL.md` packages, but it must not require a Handrails-only skill format
...
### 5.4 Base Package Vs Handrails Extension
...
- base package:
  ...
  - no Handrails-only runtime fields by default
- Handrails extension:
  - `.handrails.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Handrails tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`handrails.manifest.json` is not part of the future package direction.
...
Handrails import/export should support these entity kinds:
...
- Handrails can treat “this set of agents came from team package X” as the imported-team model
...
Handrails should support source references in package metadata with:
...
Handrails should define a new adapter surface area around skills:
...
Planned Handrails behavior:

- if an adapter supports read, Handrails should show current skills in the UI
- if an adapter supports write, Handrails should let the user enable/disable imported skills
- if an adapter supports sync, Handrails should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.handrails.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.handrails.yaml` alongside the base package
...
## 10. Storage Model Inside Handrails
...
Handrails should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `handrailsai company export <company-id> --out <path>`
- `handrailsai company import <path-or-url> --dry-run`
- `handrailsai company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Handrails storage, or only referenced at import time?
...
3. Should Handrails support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Handrails aligned with:
