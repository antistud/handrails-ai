...
- New config/env settings:
  - `HANDRAILS_AGENT_JWT_SECRET`
  - `HANDRAILS_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `HANDRAILS_AGENT_JWT_ISSUER` (default: `handrails`)
  - `HANDRAILS_AGENT_JWT_AUDIENCE` (default: `handrails-api`)
...
1. In:
   ...
   inject `HANDRAILS_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `HANDRAILS_API_KEY`, do not overwrite it.
- Continue injecting:
  - `HANDRAILS_AGENT_ID`
...
- Local adapters authenticate without manual `HANDRAILS_API_KEY` config.
