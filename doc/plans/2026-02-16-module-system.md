...
Handrails's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Handrails's API, UI, and data model | Yes |
...
modules/
  observability/
    handrails.module.json     # manifest (required)
    src/
...
### Manifest (`handrails.module.json`)
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@handrailsai/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@handrailsai/core";
...
```
1. Scan modules/ directory for handrails.module.json manifests
...
```jsonc
// handrails.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@handrailsai/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@handrailsai/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:handrails/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.handrails.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm handrailsai store list                    # browse available modules and templates
pnpm handrailsai store install <module-id>     # install a module
pnpm handrailsai store import <template-id>    # import a company template
pnpm handrailsai store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Handrails instance | `server:started` |
...
4. **Module config** — `handrails.config.json` with per-module config, env var interpolation
...
10. **`@handrailsai/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
