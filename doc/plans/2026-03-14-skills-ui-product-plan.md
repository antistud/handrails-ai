...
This document defines the product and UI plan for skill management in Handrails.
...
- Handrails company import/export can include skills as package content
...
3. The UI must always tell the truth about what Handrails knows:
   - desired state in Handrails
...
Handrails should treat skills at two scopes:
...
- created locally inside Handrails later
...
7. “Understand what Handrails knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Handrails-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Handrails did not install.

These should be clearly marked:

- external
- not managed by Handrails
...
- `desired_only`: Handrails wants it, adapter does not show it yet
- `external`: adapter has it but Handrails does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Handrails only tracks desired state
...
- Handrails can still track desired skills
...
- Handrails can see adapter skills
...
- `.handrails.yaml` may add Handrails-specific fidelity, but should not replace the base shortname association model
...
- `.handrails.yaml` may add Handrails-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Handrails should have a company-scoped skill model or managed package model representing:
...
Handrails should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Handrails-specific.
...
That gives Handrails one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
