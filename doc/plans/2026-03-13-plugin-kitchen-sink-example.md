...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Handrails plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Handrails.
...
- package: `@handrailsai/plugin-kitchen-sink-example`
- plugin id: `handrails.kitchen-sink-example` or `handrails-kitchen-sink-example`

Recommendation: use `handrails-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Handrails event such as `issue.created`
...
Recommendation: default to a Handrails-local endpoint or a stable public echo endpoint to avoid flaky docs.
