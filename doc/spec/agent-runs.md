...
This spec defines how Handrails actually runs agents while staying runtime-agnostic.
...
1. Handrails is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Handrails manages lifecycle, and on exit Handrails parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `handrails.skill` (shared Handrails skill text block)
...
2. Preferred transport is env vars (`HANDRAILS_*`) injected at runtime.
