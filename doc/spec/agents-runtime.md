...
For `claude_local` and `codex_local`, Handrails assumes the CLI is already installed and authenticated on the host machine.
...
Handrails stores resumable session state per `(agent, taskKey, adapterType)`.
