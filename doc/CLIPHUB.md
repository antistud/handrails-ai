...
ClipHub is the public registry where people share, discover, and download Handrails company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Handrails what a package registry is to a programming language. Handrails already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Handrails — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Handrails instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Handrails spec. It contains:
...
**Install (fresh start):**
```
handrails install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Handrails instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Handrails, export your company as a template, then publish:

```
handrails export --template my-company
handrails publish cliphub my-company
```
...
ClipHub is a **separate service** from Handrails itself. Handrails is self-hosted; ClipHub is a hosted registry that Handrails instances talk to.
...
| **Handrails CLI** | `handrailsai install`, `handrailsai publish`, `handrailsai cliphub sync` — built into Handrails |
| **Handrails UI** | "Browse ClipHub" panel in the Handrails web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Handrails) |
| Backend | TypeScript + Hono (consistent with Handrails) |
| Database | PostgreSQL |
...
4. Run `handrailsai install cliphub:acme/lean-saas-shop`
5. Handrails creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Handrails until it works well
2. Export: `handrailsai export --template my-agency`
3. Publish: `handrailsai publish cliphub my-agency`
...
3. Install just that agent: `handrailsai install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Handrails

ClipHub is **not required** to use Handrails. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Handrails what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `handrailsai install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Handrails web UI
- [ ] `handrailsai cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Handrails's own philosophy)
