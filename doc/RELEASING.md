...
Maintainer runbook for shipping Handrails across npm, GitHub, and the website-facing changelog surface.
...
Handrails uses calendar versions that still fit semver syntax:
...
2. **npm** — `handrailsai` and public workspace packages are published
...
```bash
npx handrailsai@canary onboard
# or
npx handrailsai@canary onboard --data-dir "$(mktemp -d /tmp/handrails-canary.XXXXXX)"
```
...
[Run the action here](https://github.com/handrailsai/handrails/actions/workflows/release.yml)
...
claude --print --output-format stream-json --verbose --dangerously-skip-permissions --model claude-opus-4-6 "Use the release-changelog skill to draft or update releases/v${VERSION}.md for Handrails. Read doc/RELEASING.md and .agents/skills/release-changelog/SKILL.md, then generate the stable changelog for v${VERSION} from commits since the last stable tag. Do not create a canary changelog."
```
...
```bash
HANDRAILSAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
```

For the current stable:

```bash
HANDRAILSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Useful isolated variants:

```bash
HOST_PORT=3232 DATA_DIR=./data/release-smoke-canary HANDRAILSAI_VERSION=canary ./scripts/docker-onboard-smoke.sh
HOST_PORT=3233 DATA_DIR=./data/release-smoke-stable HANDRAILSAI_VERSION=latest ./scripts/docker-onboard-smoke.sh
```

Automated browser smoke is also available:

```bash
gh workflow run release-smoke.yml -f handrails_version=canary
gh workflow run release-smoke.yml -f handrails_version=latest
```

Minimum checks:

- `npx handrailsai@canary onboard` installs
