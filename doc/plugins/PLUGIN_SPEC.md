# Handrails Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Handrails's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Handrails app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Handrails HTTP APIs directly.
...
Handrails plugin design is based on the following assumptions:

1. Handrails is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Handrails business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Handrails core.
...
2. Let plugins add major capabilities without editing Handrails core.
...
4. Depend on project-local plugin folders such as `.handrails/plugins`.
...
The single Handrails deployment an operator installs and controls.

### 5.2 Company

A first-class Handrails business object inside the instance.
...
A trusted in-process extension loaded directly by Handrails core.
...
An installable instance-wide extension package loaded through the Handrails plugin runtime.
...
Handrails has two extension classes.
...
Handrails already has a concrete workspace model:
...
Plugins live under the Handrails instance directory.

Suggested layout:

- `~/.handrails/instances/default/plugins/package.json`
- `~/.handrails/instances/default/plugins/node_modules/`
- `~/.handrails/instances/default/plugins/.cache/`
- `~/.handrails/instances/default/data/plugins/<plugin-id>/`
...
Handrails should add CLI commands:

- `pnpm handrailsai plugin list`
- `pnpm handrailsai plugin install <package[@version]>`
- `pnpm handrailsai plugin uninstall <plugin-id>`
- `pnpm handrailsai plugin upgrade <plugin-id> [version]`
- `pnpm handrailsai plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@handrails/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
```ts
export interface HandrailsPluginManifestV1 {
  id: string;
  apiVersion: 1;
  version: string;
  displayName: string;
  ...
  minimumHandrailsVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
Plugins may contribute tools that Handrails agents can use during runs.
...
- Handrails server starts one worker process per installed plugin
...
Receives one typed Handrails domain event.
...
export function definePlugin(definition: PluginDefinition): HandrailsPlugin;

/** Re-exported from Zod for config schema definitions */
export { z } from "zod";

export interface PluginContext {
  manifest: HandrailsPluginManifestV1;
  config: {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@handrailsai/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@handrailsai/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@handrailsai/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@handrailsai/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@handrailsai/plugin-sdk/ui`
...
The `@handrailsai/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Handrails secret provider system rather than a plain text input
...
1. Core Handrails data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Handrails objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Handrails product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Handrails later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Handrails secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm handrailsai plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Handrails server. This is a normative requirement, not optional.
...
### 27.1 `@handrailsai/plugin-test-harness`
...
```ts
import { createTestHarness } from "@handrailsai/plugin-test-harness";
...
For developing a plugin against a running Handrails instance:

- The operator installs the plugin from a local path: `pnpm handrailsai plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-handrails-plugin`) that scaffolds:

- `package.json` with correct `handrailsPlugin` keys
...
- `@handrails/plugin-workspace-files`
- `@handrails/plugin-terminal`
- `@handrails/plugin-git`
- `@handrails/plugin-linear`
- `@handrails/plugin-github-issues`
- `@handrails/plugin-grafana`
- `@handrails/plugin-runtime-processes`
- `@handrails/plugin-stripe`
...
- `@handrailsai/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@handrailsai/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@handrailsai/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@handrailsai/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@handrailsai/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@handrailsai/plugin-sdk/ui`
...
- `@handrailsai/plugin-test-harness`
- `create-handrails-plugin` starter template
...
Handrails should not implement a generic in-process hook bag modeled directly after local coding tools.

Handrails should implement:
...
That is the complete target design for the Handrails plugin system.
