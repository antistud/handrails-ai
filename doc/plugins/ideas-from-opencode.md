...
Handrails V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Handrails-shaped design.

Assumption for this document: Handrails is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Handrails objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Handrails.

The main conclusion is:

- Handrails should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Handrails should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Handrails should use multiple extension classes instead of one generic plugin bag:
...
If Handrails does this well, the examples you listed become straightforward:
...
Relevant Handrails files reviewed for current extension seams:
...
It is too dangerous for Handrails core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Handrails — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Handrails should absolutely do this.
...
Handrails should copy this discipline.
...
Handrails should adopt this style for plugin actions, automations, and UI schemas.
...
Handrails can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Handrails too.

## What Handrails Should Not Copy Directly
...
Handrails is a control plane for an operator-managed instance with company objects.
...
Handrails is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Handrails should not auto-load arbitrary code from a workspace repo like `.handrails/plugins` or project directories.
...
For Handrails, equivalent hooks into:
...
Handrails should not allow plugins to silently replace:
...
For Handrails it would be risky because it combines:
...
Handrails should require an explicit operator install step.

## Why Handrails Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Handrails |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Handrails instance |
...
That means Handrails should borrow the good ideas from `opencode` but use a stricter architecture.

## Handrails Already Has Useful Pre-Plugin Seams

Handrails has several extension-like seams already:
...
This is good news.
Handrails does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Handrails Plugin Model
...
Handrails already has this pattern implicitly:
...
- `plugin` means integration code that talks to Handrails through a typed plugin protocol and capability model
...
- invoke explicit Handrails actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@handrailsai/plugin-sdk/ui`) with design tokens and common components.
...
Handrails should treat plugin installation as a global instance-level action.

Examples:

- install `@handrails/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Handrails objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Handrails already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Handrails too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@handrails/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@handrailsai/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Handrails instance.

A `create-handrails-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Handrails achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Handrails server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Handrails's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@handrailsai/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@handrailsai/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Handrails

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@handrailsai/plugin-sdk";

export default definePlugin({
  id: "@handrails/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit Handrails entities
    });
    ...
    ctx.events.on("plugin.@handrails/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@handrailsai/plugin-sdk/ui";
...
Any Handrails plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Handrails objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@handrails/plugin-linear`)
...
- reuse core tables when the data is clearly part of Handrails itself
...
If a concept is becoming part of Handrails's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Handrails itself
...
### 3. Scope plugin data to Handrails objects before adding custom schemas

A lot of plugin data naturally hangs off existing Handrails objects:
...
If Handrails eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@handrails/plugin-workspace-files`
...
Package idea: `@handrails/plugin-terminal`
...
Package idea: `@handrails/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Handrails.
- Board reviews the diff after a run without leaving Handrails.
...
- optional `events.emit` (e.g. `plugin.@handrails/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@handrails/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@handrails/plugin-linear`

This plugin syncs Handrails work with Linear. It is useful for:

- importing backlog from Linear
- linking Handrails issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Handrails and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Handrails.
- Handrails issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@handrails/plugin-github-issues`

This plugin syncs Handrails issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Handrails issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Handrails task governance
...
- Plugin settings:
  ...
  - whether new Handrails issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Handrails issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Handrails.
- GitHub webhooks update status/comment state in Handrails.
- A PR is linked back to the Handrails issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@handrails/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@handrails/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Handrails. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Handrails issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Handrails plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Handrails dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Handrails issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Handrails core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@handrails/plugin-runtime-processes`
...
Package idea: `@handrails/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Handrails. It is useful for:
...
- Failed payment spikes or churn events can generate Handrails issues for follow-up.
...
- Stripe data should stay additive to Handrails core
...
- plugin UI bundle loading, host bridge, `@handrailsai/plugin-sdk/ui`
...
- `@handrailsai/plugin-test-harness` and `create-handrails-plugin` starter template
...
Handrails should not implement "an OpenCode-style generic in-process hook system."
Handrails should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Handrails
...
8. Ship `@handrailsai/plugin-test-harness` and `create-handrails-plugin` starter template.
