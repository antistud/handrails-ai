...
This guide describes the current, implemented way to create a Handrails plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Handrails app.
...
```bash
pnpm --filter @handrailsai/create-handrails-plugin build
node packages/plugins/create-handrails-plugin/dist/index.js @yourscope/plugin-name --output ./packages/plugins/examples
```

For a plugin that lives outside the Handrails repo:

```bash
pnpm --filter @handrailsai/create-handrails-plugin build
node packages/plugins/create-handrails-plugin/dist/index.js @yourscope/plugin-name \
...
Inside this monorepo, the scaffold uses `workspace:*` for `@handrailsai/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@handrailsai/plugin-sdk` from the local Handrails checkout into a `.handrails-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
For local development, install it into Handrails from an absolute local path through the plugin manager or API. The server supports local filesystem installs and watches local-path plugins for file changes so worker restarts happen automatically after rebuilds.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@handrailsai/plugin-sdk/ui`
