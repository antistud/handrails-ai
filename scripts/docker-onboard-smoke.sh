...
IMAGE_NAME="${IMAGE_NAME:-handrails-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
HANDRAILSAI_VERSION="${HANDRAILSAI_VERSION:-latest}"
...
HANDRAILS_DEPLOYMENT_MODE="${HANDRAILS_DEPLOYMENT_MODE:-authenticated}"
HANDRAILS_DEPLOYMENT_EXPOSURE="${HANDRAILS_DEPLOYMENT_EXPOSURE:-private}"
HANDRAILS_PUBLIC_URL="${HANDRAILS_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@handrails.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-handrails-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_HANDRAILSAI_VERSION=%q\n' "$HANDRAILSAI_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e HANDRAILS_DEPLOYMENT_MODE="$HANDRAILS_DEPLOYMENT_MODE" \
      -e HANDRAILS_DEPLOYMENT_EXPOSURE="$HANDRAILS_DEPLOYMENT_EXPOSURE" \
      -e HANDRAILS_PUBLIC_URL="$HANDRAILS_PUBLIC_URL" \
      -e HANDRAILS_HOME="/handrails" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "handrailsai@${HANDRAILSAI_VERSION}" auth bootstrap-ceo --data-dir "$HANDRAILS_HOME" --base-url "$HANDRAILS_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$HANDRAILS_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$HANDRAILS_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$HANDRAILS_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$HANDRAILS_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$HANDRAILS_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$HANDRAILS_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg HANDRAILSAI_VERSION="$HANDRAILSAI_VERSION" \
...
echo "    Public URL: $HANDRAILS_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $HANDRAILS_DEPLOYMENT_MODE/$HANDRAILS_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/handrails-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$HANDRAILS_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $HANDRAILS_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$HANDRAILS_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $HANDRAILS_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
