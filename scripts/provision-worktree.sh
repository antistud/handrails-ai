...
base_cwd="${HANDRAILS_WORKSPACE_BASE_CWD:?HANDRAILS_WORKSPACE_BASE_CWD is required}"
worktree_cwd="${HANDRAILS_WORKSPACE_CWD:?HANDRAILS_WORKSPACE_CWD is required}"
handrails_home="${HANDRAILS_HOME:-$HOME/.handrails}"
handrails_instance_id="${HANDRAILS_INSTANCE_ID:-default}"
handrails_dir="$worktree_cwd/.handrails"
worktree_config_path="$handrails_dir/config.json"
worktree_env_path="$handrails_dir/.env"
worktree_name="${HANDRAILS_WORKSPACE_BRANCH:-$(basename "$worktree_cwd")}"
...
source_config_path="${HANDRAILS_CONFIG:-}"
if [[ -z "$source_config_path" && ( -e "$base_cwd/.handrails/config.json" || -L "$base_cwd/.handrails/config.json" ) ]]; then
  source_config_path="$base_cwd/.handrails/config.json"
fi
if [[ -z "$source_config_path" ]]; then
  source_config_path="$handrails_home/instances/$handrails_instance_id/config.json"
fi
source_env_path="$(dirname "$source_config_path")/.env"

mkdir -p "$handrails_dir"

run_isolated_worktree_init() {
  if command -v pnpm >/dev/null 2>&1 && pnpm handrailsai --help >/dev/null 2>&1; then
    pnpm handrailsai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi

  if command -v handrailsai >/dev/null 2>&1; then
    handrailsai worktree init --force --seed-mode minimal --name "$worktree_name" --from-config "$source_config_path"
    return 0
  fi
...
write_fallback_worktree_config() {
  ...
  HANDRAILS_DIR="$handrails_dir" \
  SOURCE_CONFIG_PATH="$source_config_path" \
  SOURCE_ENV_PATH="$source_env_path" \
  HANDRAILS_WORKTREES_DIR="${HANDRAILS_WORKTREES_DIR:-}" \
  node <<'EOF'
...
async function main() {
  ...
  const handrailsDir = process.env.HANDRAILS_DIR;
  const sourceConfigPath = process.env.SOURCE_CONFIG_PATH;
  const sourceEnvPath = process.env.SOURCE_ENV_PATH;
  const worktreeHome = path.resolve(expandHomePrefix(nonEmpty(process.env.HANDRAILS_WORKTREES_DIR) ?? "~/.handrails-worktrees"));
  const instanceId = sanitizeInstanceId(worktreeName);
  const instanceRoot = path.resolve(worktreeHome, "instances", instanceId);
  const configPath = path.resolve(handrailsDir, "config.json");
  const envPath = path.resolve(handrailsDir, ".env");
  ...
  const targetConfig = {
    ...
    storage: {
      ...
      s3: {
        bucket: sourceConfig?.storage?.s3?.bucket ?? "handrails",
  ...
  const inlineMasterKey = nonEmpty(sourceEnvEntries.HANDRAILS_SECRETS_MASTER_KEY);
  if (inlineMasterKey) {
    ...
    const sourceKeyFilePath = nonEmpty(sourceEnvEntries.HANDRAILS_SECRETS_MASTER_KEY_FILE)
      ? resolveRuntimeLikePath(sourceEnvEntries.HANDRAILS_SECRETS_MASTER_KEY_FILE, sourceConfigPath)
  ...
  const envLines = [
    "HANDRAILS_HOME=" + JSON.stringify(worktreeHome),
    "HANDRAILS_INSTANCE_ID=" + JSON.stringify(instanceId),
    "HANDRAILS_CONFIG=" + JSON.stringify(configPath),
    "HANDRAILS_CONTEXT=" + JSON.stringify(path.resolve(worktreeHome, "context.json")),
    "HANDRAILS_IN_WORKTREE=true",
    "HANDRAILS_WORKTREE_NAME=" + JSON.stringify(worktreeName),
  ];

  const agentJwtSecret = nonEmpty(sourceEnvEntries.HANDRAILS_AGENT_JWT_SECRET);
  if (agentJwtSecret) {
    envLines.push("HANDRAILS_AGENT_JWT_SECRET=" + JSON.stringify(agentJwtSecret));
  }
...
if ! run_isolated_worktree_init; then
  echo "handrailsai CLI not available in this workspace; writing isolated fallback config without DB seeding." >&2
  write_fallback_worktree_config
fi
