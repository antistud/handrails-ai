...
Environment overrides:
  ...
  PC_DATA=/tmp/data           Handrails data dir to use
  HANDRAILS_HOST=127.0.0.1    Host passed to the onboarded server
  HANDRAILS_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/handrails-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/handrails-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export HANDRAILS_HOST="${HANDRAILS_HOST:-127.0.0.1}"
export HANDRAILS_PORT="${HANDRAILS_PORT:-3100}"
export HANDRAILS_OPEN_ON_LISTEN="${HANDRAILS_OPEN_ON_LISTEN:-false}"
...
echo "HANDRAILS_HOST: $HANDRAILS_HOST"
echo "HANDRAILS_PORT: $HANDRAILS_PORT"
...
env \
  ...
  HOST="$HANDRAILS_HOST" \
  PORT="$HANDRAILS_PORT" \
  HANDRAILS_OPEN_ON_LISTEN="$HANDRAILS_OPEN_ON_LISTEN" \
  pnpm handrailsai onboard --yes --data-dir "$PC_DATA"
