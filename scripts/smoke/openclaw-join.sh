...
COMPANY_ID="${COMPANY_ID:-${HANDRAILS_COMPANY_ID:-}}"
...
SMOKE_IMAGE="${SMOKE_IMAGE:-handrails-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-handrails-openclaw-smoke}"
...
Provide one of:
  HANDRAILS_AUTH_HEADER=\"Bearer <board-token>\"
  HANDRAILS_COOKIE=\"<board-session-cookie>\"
...
assert_status "200"
if ! grep -q "Handrails OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.handrails.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
