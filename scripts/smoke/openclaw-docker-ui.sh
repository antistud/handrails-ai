...
HANDRAILS_HOST_PORT="${HANDRAILS_HOST_PORT:-3100}"
HANDRAILS_HOST_FROM_CONTAINER="${HANDRAILS_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.handrails-openclaw.override.yml"
...
detect_handrails_base_url() {
  ...
  for candidate in "$HANDRAILS_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${HANDRAILS_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${HANDRAILS_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
handrails_base_url="$(detect_handrails_base_url || true)"
...
Handrails URL for OpenClaw container:
EOF
  if [[ -n "$handrails_base_url" ]]; then
    cat <<EOF
  $handrails_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${HANDRAILS_HOST_PORT}
  (Do not use http://127.0.0.1:${HANDRAILS_HOST_PORT} inside the container.)
  If Handrails rejects the host, run on host machine:
    pnpm handrailsai allowed-hostname host.docker.internal
  Then restart Handrails and re-run this script.
EOF
  fi
  cat <<EOF
