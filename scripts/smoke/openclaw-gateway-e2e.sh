...
HANDRAILS_API_URL="${HANDRAILS_API_URL:-http://127.0.0.1:3100}"
API_BASE="${HANDRAILS_API_URL%/}/api"
...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-handrails-smoke}"
...
HANDRAILS_API_URL_FOR_OPENCLAW="${HANDRAILS_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
AUTH_HEADERS=()
if [[ -n "${HANDRAILS_AUTH_HEADER:-}" ]]; then
  AUTH_HEADERS+=( -H "Authorization: ${HANDRAILS_AUTH_HEADER}" )
fi
if [[ -n "${HANDRAILS_COOKIE:-}" ]]; then
  AUTH_HEADERS+=( -H "Cookie: ${HANDRAILS_COOKIE}" )
  HANDRAILS_BROWSER_ORIGIN="${HANDRAILS_BROWSER_ORIGIN:-${HANDRAILS_API_URL%/}}"
  AUTH_HEADERS+=( -H "Origin: ${HANDRAILS_BROWSER_ORIGIN}" -H "Referer: ${HANDRAILS_BROWSER_ORIGIN}/" )
fi
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set HANDRAILS_COOKIE or HANDRAILS_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg handrailsApiUrl "$HANDRAILS_API_URL_FOR_OPENCLAW" \
    ...
    '{
      ...
      agentDefaultsPayload: {
        ...
        sessionKey: "handrails",
        timeoutSec: $timeoutSec,
        waitTimeoutMs: $waitTimeoutMs,
        handrailsApiUrl: $handrailsApiUrl
      }
    }')"
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/handrails"
  local claimed_file="${workspace_dir}/handrails-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/handrails-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/handrails-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/handrails-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/handrails"
    assert_status "200"
    {
      echo "HANDRAILS_API_URL=${HANDRAILS_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set HANDRAILS_API_KEY=${AGENT_API_KEY} in your run context before running Handrails heartbeat steps."
...
main() {
  ...
  wait_http_ready "${HANDRAILS_API_URL%/}/api/health" 15 || fail "Handrails API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "handrails health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
