...
const DEFAULT_QUERY = "\"Co-Authored-By: Handrails <noreply@handrails.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/handrails-commit-metrics-cache.json");
...
const HANDRAILS_EMAIL = "noreply@handrails.ing";
const HANDRAILS_NAME = "handrails";
...
function printHelp() {
  console.log(`Usage: tsx scripts/handrails-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === HANDRAILS_EMAIL) || (displayName && displayName.toLowerCase() === HANDRAILS_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("Handrails commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "handrails-commit-metrics",
